...
function _configHash(a){let s=JSON.stringify(a),h=0;for(let i=0;i<s.length;i++){h=((h<<5)-h)+s.charCodeAt(i);h|=0;}return Math.abs(h).toString(16).substring(0,8);}
function finishGen(_errs){
    if(_errs.length){
      console.error('Generation errors:',_errs);
      toast(S.lang==='ja'?'⚠️ 一部のファイル生成に失敗しました（'+_errs.length+'件）。もう一度「生成」ボタンを押してください。繰り返す場合は回答を少し変更してみてください。':'⚠️ Some files failed to generate ('+_errs.length+'). Please try again; if it persists, adjust your answers slightly.');
    }
    // C-7: Inject generation metadata into all generated files
    const _metaTs=new Date().toISOString();
    const _metaHash=_configHash(S.answers);
    const _metaComment='<!-- Generated by DevForge v9.39.0 | '+_metaTs+' | Config: '+_metaHash+' -->';
    const _metaYAML='# Generated by DevForge v9.39.0 | '+_metaTs+' | Config: '+_metaHash;
    Object.keys(S.files).forEach(path=>{
      if(path.endsWith('.md')&&S.files[path]){S.files[path]=_metaComment+'\n'+S.files[path];}
      else if((path.endsWith('.yaml')||path.endsWith('.yml'))&&S.files[path]){S.files[path]=_metaYAML+'\n'+S.files[path];}
    });
