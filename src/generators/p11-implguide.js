...
function genPillar11_ImplIntelligence(a,pn){
  ...
  const domainErrors={
    ...
    saas:{ja:'テナント境界越え → RLSポリシー再確認',en:'Cross-tenant access → Re-verify RLS policies'},
    community:{ja:'禁止語バイパス → モデレーションフィルタのエスケープ確認',en:'Banned word bypass → Check moderation filter escape paths'},
    booking:{ja:'重複予約 → 悲観的ロックまたは一意制約の追加',en:'Duplicate booking → Add pessimistic lock or unique constraint'},
    iot:{ja:'センサー値異常 → 入力範囲バリデーション追加',en:'Sensor anomaly → Add input range validation rules'},
    realestate:{ja:'物件重複登録 → (location+type)複合一意インデックス追加',en:'Duplicate property → Add unique index on (location+type)'},
    legal:{ja:'機密文書不正アクセス → RLSポリシーと所有権チェック強化',en:'Unauthorized confidential access → Strengthen RLS and ownership check'},
    hr:{ja:'個人情報漏洩 → テナント分離とRLS確認',en:'Personal data leak → Verify tenant isolation and RLS'},
    analytics:{ja:'集計値不一致 → 非同期集計のキャッシュ無効化確認',en:'Aggregation mismatch → Check cache invalidation for async aggregation'},
    ai:{ja:'プロンプトインジェクション → 入力サニタイズと出力検証強化',en:'Prompt injection → Strengthen input sanitization and output validation'},
    automation:{ja:'ワークフロー無限ループ → 循環依存チェックと最大実行回数設定',en:'Workflow infinite loop → Check circular deps and set max execution count'},
    gamify:{ja:'ポイント不正付与 → サーバーサイドのみ集計・クライアント値無視',en:'Point fraud → Server-side only aggregation, ignore client values'},
    collab:{ja:'編集競合でデータ消失 → OT/CRDTアルゴリズム適用確認',en:'Edit conflict data loss → Verify OT/CRDT algorithm application'},
    creator:{ja:'収益計算誤差 → 浮動小数点を整数(centベース)に変換',en:'Revenue calculation error → Convert float to integer (cent-based)'},
    newsletter:{ja:'配信重複 → 送信済みIDセットによる冪等性チェック',en:'Duplicate delivery → Idempotency check via sent ID set'},
    travel:{ja:'過剰予約 → 在庫確認と予約確定をアトミックトランザクション化',en:'Overbooking → Atomize inventory check and booking confirmation'},
    government:{ja:'個人情報不正参照 → ロールベースRLS + アクセスログ必須',en:'Unauthorized personal data access → Role-based RLS + mandatory access log'},
    insurance:{ja:'クレーム金額計算ミス → 保険証券条件のサーバーサイド再検証',en:'Claim amount error → Re-validate policy terms server-side'},
    manufacturing:{ja:'品質データ改ざん → 変更不可フラグ + 監査ログ設定',en:'Quality data tampering → Set immutable flag + audit log'},
  ...
  // Footer
  doc42+='---\n';
  doc42+=(G?'*このガイドはDevForge v9.39が `S.skill='+skill+'` に基づいて自動生成しました。*\n':'*This guide was auto-generated by DevForge v9.39 based on `S.skill='+skill+'`.*\n');
