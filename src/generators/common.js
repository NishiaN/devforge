...
const ENTITY_COLUMNS={
  ...
  Lesson:['sort_order:INT:DEFAULT 0:表示順:Display order','content_type:VARCHAR(20):DEFAULT \'text\':コンテンツ種別:Content type','duration_min:INT::所要時間(分):Duration(min)','video_url:TEXT::動画URL:Video URL','is_free:BOOLEAN:DEFAULT false:無料公開:Free access'],
  Progress:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','lesson_id:UUID:FK(Lesson) NOT NULL:レッスンID:Lesson ID','status:VARCHAR(20):DEFAULT \'not_started\':進捗状態:Progress status','completed_at:TIMESTAMP::完了日時:Completed at','score:INT::スコア:Score'],
  ...
  Enrollment:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','course_id:UUID:FK(Course) NOT NULL:コースID:Course ID','enrolled_at:TIMESTAMP:DEFAULT NOW:受講開始日:Enrolled at','expires_at:TIMESTAMP::有効期限:Expires at','status:VARCHAR(20):DEFAULT \'active\':受講状態:Enrollment status'],
  Certificate:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','course_id:UUID:FK(Course) NOT NULL:コースID:Course ID','issued_at:TIMESTAMP:DEFAULT NOW:発行日:Issued at','certificate_url:TEXT::証明書URL:Certificate URL'],
  Product:['description:TEXT::商品説明:Product description','price:DECIMAL(10,2):NOT NULL:価格:Price','stock:INT:DEFAULT 0:在庫数:Stock','sku:VARCHAR(100):UNIQUE:SKU:SKU','image_url:TEXT::商品画像:Product image','category_id:UUID:FK(Category):カテゴリID:Category ID','status:VARCHAR(20):DEFAULT \'active\':販売状態:Status'],
  Category:['slug:VARCHAR(100):UNIQUE:スラグ:Slug','description:TEXT::カテゴリ説明:Description','parent_id:UUID:FK(Category):親カテゴリ:Parent category','sort_order:INT:DEFAULT 0:表示順:Display order'],
  Order:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','total:DECIMAL(10,2):NOT NULL:合計金額:Total','status:VARCHAR(20):DEFAULT \'pending\':注文状態:Order status','stripe_session_id:TEXT::Stripe Session ID:Stripe Session ID','shipping_address:JSONB::配送先:Shipping address'],
  Cart:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','product_id:UUID:FK(Product) NOT NULL:商品ID:Product ID','quantity:INT:DEFAULT 1:数量:Quantity'],
  Review:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','rating:INT:NOT NULL:評価(1-5):Rating(1-5)','body:TEXT::レビュー本文:Review body'],
  Post:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','title:VARCHAR(255):NOT NULL:タイトル:Title','body:TEXT::本文:Body','status:VARCHAR(20):DEFAULT \'draft\':公開状態:Status','published_at:TIMESTAMP::公開日時:Published at','slug:VARCHAR(255):UNIQUE:スラグ:Slug'],
  Comment:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','post_id:UUID:FK(Post) NOT NULL:投稿ID:Post ID','body:TEXT:NOT NULL:コメント本文:Comment body'],
  Tag:['slug:VARCHAR(100):UNIQUE NOT NULL:スラグ:Slug'],
  Media:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','url:TEXT:NOT NULL:ファイルURL:File URL','mime_type:VARCHAR(50)::MIMEタイプ:MIME type','size_bytes:BIGINT::ファイルサイズ:File size'],
  Like:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','post_id:UUID:FK(Post) NOT NULL:投稿ID:Post ID'],
  Follow:['follower_id:UUID:FK(User) NOT NULL:フォロワーID:Follower ID','following_id:UUID:FK(User) NOT NULL:フォロー対象ID:Following ID'],
  Message:['sender_id:UUID:FK(User) NOT NULL:送信者ID:Sender ID','receiver_id:UUID:FK(User) NOT NULL:受信者ID:Receiver ID','body:TEXT:NOT NULL:メッセージ本文:Message body','read_at:TIMESTAMP::既読日時:Read at'],
  Listing:['user_id:UUID:FK(User) NOT NULL:出品者ID:Seller ID','title:VARCHAR(255):NOT NULL:タイトル:Title','description:TEXT::説明:Description','price:DECIMAL(10,2):NOT NULL:価格:Price','status:VARCHAR(20):DEFAULT \'active\':出品状態:Status','category_id:UUID:FK(Category):カテゴリID:Category ID'],
  Service:['provider_id:UUID:FK(User) NOT NULL:提供者ID:Provider ID','description:TEXT::サービス説明:Description','price:DECIMAL(10,2)::料金:Price','duration_min:INT::所要時間(分):Duration(min)'],
  Booking:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','service_id:UUID:FK(Service) NOT NULL:サービスID:Service ID','starts_at:TIMESTAMP:NOT NULL:開始日時:Starts at','ends_at:TIMESTAMP:NOT NULL:終了日時:Ends at','status:VARCHAR(20):DEFAULT \'pending\':予約状態:Status'],
  ...
  Project:['workspace_id:UUID:FK(Workspace):ワークスペースID:Workspace ID','status:VARCHAR(20):DEFAULT \'active\':プロジェクト状態:Status','description:TEXT::説明:Description'],
  ...
  Subscription:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','stripe_subscription_id:TEXT:UNIQUE:StripeサブスクID:Stripe Sub ID','stripe_customer_id:TEXT::Stripe顧客ID:Stripe Customer ID','status:VARCHAR(20):DEFAULT \'active\':契約状態:Status','plan:VARCHAR(20):NOT NULL:プラン名:Plan name','current_period_end:TIMESTAMP::現在期間終了:Current period end'],
  Invoice:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','amount:DECIMAL(10,2):NOT NULL:金額:Amount','status:VARCHAR(20):DEFAULT \'pending\':請求状態:Status','stripe_invoice_id:TEXT::Stripe請求ID:Stripe Invoice ID','paid_at:TIMESTAMP::支払日:Paid at'],
  Payment:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','amount:DECIMAL(10,2):NOT NULL:金額:Amount','method:VARCHAR(20)::支払方法:Payment method','status:VARCHAR(20):DEFAULT \'pending\':支払状態:Status','stripe_payment_id:TEXT::Stripe決済ID:Stripe Payment ID'],
  Dashboard:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','layout:JSONB::レイアウト設定:Layout config'],
  Widget:['dashboard_id:UUID:FK(Dashboard) NOT NULL:ダッシュボードID:Dashboard ID','type:VARCHAR(50):NOT NULL:ウィジェット種別:Widget type','config:JSONB::設定:Config','position:INT:DEFAULT 0:表示位置:Position'],
  ...
  Device:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','device_name:VARCHAR(255):NOT NULL:デバイス名:Device name','device_type:VARCHAR(50)::デバイス種別:Device type','status:VARCHAR(20):DEFAULT \'active\':状態:Status'],
  ...
  Alert:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','severity:VARCHAR(20):DEFAULT \'info\':重要度:Severity','message:TEXT:NOT NULL:メッセージ:Message','acknowledged_at:TIMESTAMP::確認日時:Acknowledged at'],
  Notification:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','title:VARCHAR(255):NOT NULL:タイトル:Title','body:TEXT::本文:Body','read_at:TIMESTAMP::既読日時:Read at','type:VARCHAR(50):DEFAULT \'general\':通知種別:Type'],
  AuditLog:['user_id:UUID:FK(User):ユーザーID:User ID','action:VARCHAR(100):NOT NULL:操作:Action','resource_type:VARCHAR(50)::対象リソース:Resource type','resource_id:UUID::対象ID:Resource ID','metadata:JSONB::メタデータ:Metadata'],
  Skill:['category:VARCHAR(100)::スキルカテゴリ:Skill category','level:INT:DEFAULT 0:レベル:Level'],
  ...
  Activity:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','action:VARCHAR(100):NOT NULL:操作種別:Action type','resource_type:VARCHAR(50)::対象リソース:Resource type','resource_id:UUID::対象ID:Resource ID','metadata:JSONB::メタデータ:Metadata'],
  // ── AI/Chat ──
  Conversation:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','title:VARCHAR(255)::会話タイトル:Conversation title','model:VARCHAR(50)::使用モデル:Model used','token_count:INT:DEFAULT 0:トークン数:Token count','status:VARCHAR(20):DEFAULT \'active\':ステータス:Status'],
  Prompt:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','title:VARCHAR(255):NOT NULL:プロンプト名:Prompt title','content:TEXT:NOT NULL:プロンプト内容:Prompt content','category:VARCHAR(50)::カテゴリ:Category','is_public:BOOLEAN:DEFAULT false:公開設定:Public'],
  ApiUsage:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','endpoint:VARCHAR(255):NOT NULL:エンドポイント:Endpoint','tokens_used:INT:NOT NULL:使用トークン:Tokens used','cost:DECIMAL(10,4)::コスト:Cost','request_at:TIMESTAMP:DEFAULT NOW:リクエスト日時:Request at'],
  Agent:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','agent_name:VARCHAR(255):NOT NULL:エージェント名:Agent name','system_prompt:TEXT::システムプロンプト:System prompt','model:VARCHAR(50)::使用モデル:Model','status:VARCHAR(20):DEFAULT \'active\':ステータス:Status','config:JSONB::設定:Config'],
  Tool:['agent_id:UUID:FK(Agent) NOT NULL:エージェントID:Agent ID','tool_name:VARCHAR(100):NOT NULL:ツール名:Tool name','tool_type:VARCHAR(50)::ツール種別:Tool type','config:JSONB::設定:Config','is_enabled:BOOLEAN:DEFAULT true:有効:Enabled'],
  Template:['user_id:UUID:FK(User):ユーザーID:User ID','title:VARCHAR(255):NOT NULL:テンプレート名:Template title','content:TEXT:NOT NULL:テンプレート内容:Content','category:VARCHAR(50)::カテゴリ:Category','usage_count:INT:DEFAULT 0:使用回数:Usage count'],
  Generation:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','template_id:UUID:FK(Template):テンプレートID:Template ID','input:TEXT::入力:Input','output:TEXT::出力:Output','model:VARCHAR(50)::使用モデル:Model','tokens_used:INT::使用トークン:Tokens used','status:VARCHAR(20):DEFAULT \'completed\':ステータス:Status'],
  Content:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','title:VARCHAR(255):NOT NULL:タイトル:Title','body:TEXT::本文:Body','content_type:VARCHAR(50)::コンテンツ種別:Content type','status:VARCHAR(20):DEFAULT \'draft\':ステータス:Status','published_at:TIMESTAMP::公開日時:Published at'],
  CreditUsage:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','credits_used:INT:NOT NULL:使用クレジット:Credits used','action:VARCHAR(100)::操作:Action','balance_after:INT::残高:Balance after'],
  // ── Automation/Workflow ──
  Workflow:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','title:VARCHAR(255):NOT NULL:ワークフロー名:Workflow name','description:TEXT::説明:Description','status:VARCHAR(20):DEFAULT \'draft\':ステータス:Status','is_active:BOOLEAN:DEFAULT false:有効:Active','config:JSONB::設定:Config'],
  Trigger:['workflow_id:UUID:FK(Workflow) NOT NULL:ワークフローID:Workflow ID','trigger_type:VARCHAR(50):NOT NULL:トリガー種別:Trigger type','config:JSONB::設定:Config','is_enabled:BOOLEAN:DEFAULT true:有効:Enabled'],
  Action:['workflow_id:UUID:FK(Workflow) NOT NULL:ワークフローID:Workflow ID','action_type:VARCHAR(50):NOT NULL:アクション種別:Action type','sort_order:INT:DEFAULT 0:実行順:Order','config:JSONB::設定:Config'],
  ...
  Connection:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','service:VARCHAR(100):NOT NULL:サービス名:Service','credentials:JSONB::認証情報:Credentials','status:VARCHAR(20):DEFAULT \'active\':ステータス:Status'],
  Log:['user_id:UUID:FK(User):ユーザーID:User ID','level:VARCHAR(20):DEFAULT \'info\':レベル:Level','message:TEXT:NOT NULL:メッセージ:Message','metadata:JSONB::メタデータ:Metadata'],
  // ── Marketplace ──
  Transaction:['buyer_id:UUID:FK(User) NOT NULL:購入者ID:Buyer ID','seller_id:UUID:FK(User) NOT NULL:販売者ID:Seller ID','amount:DECIMAL(10,2):NOT NULL:金額:Amount','fee:DECIMAL(10,2):DEFAULT 0:手数料:Fee','status:VARCHAR(20):DEFAULT \'pending\':取引状態:Status','stripe_transfer_id:TEXT::Stripe送金ID:Stripe transfer ID'],
  // ── Chatbot ──
  Bot:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','bot_name:VARCHAR(255):NOT NULL:ボット名:Bot name','welcome_message:TEXT::ウェルカムメッセージ:Welcome message','model:VARCHAR(50)::使用モデル:Model','status:VARCHAR(20):DEFAULT \'active\':ステータス:Status'],
  ...
  KnowledgeBase:['bot_id:UUID:FK(Bot) NOT NULL:ボットID:Bot ID','title:VARCHAR(255):NOT NULL:タイトル:Title','content:TEXT:NOT NULL:内容:Content','embedding:JSONB::埋め込みベクトル:Embedding vector','source_url:TEXT::ソースURL:Source URL'],
  Handoff:['conversation_id:UUID:FK(Conversation) NOT NULL:会話ID:Conversation ID','agent_id:UUID:FK(User):担当者ID:Agent ID','reason:TEXT::理由:Reason','status:VARCHAR(20):DEFAULT \'pending\':ステータス:Status','resolved_at:TIMESTAMP::解決日時:Resolved at'],
  // ── Fintech ──
  Account:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','account_name:VARCHAR(255):NOT NULL:口座名:Account name','account_type:VARCHAR(50)::口座種別:Account type','balance:DECIMAL(12,2):DEFAULT 0:残高:Balance','currency:VARCHAR(3):DEFAULT \'JPY\':通貨:Currency','institution:VARCHAR(255)::金融機関:Institution'],
  Budget:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','category:VARCHAR(100):NOT NULL:カテゴリ:Category','amount:DECIMAL(10,2):NOT NULL:予算額:Budget amount','period:VARCHAR(20):DEFAULT \'monthly\':期間:Period','spent:DECIMAL(10,2):DEFAULT 0:使用額:Spent'],
  Report:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','title:VARCHAR(255):NOT NULL:レポート名:Report title','report_type:VARCHAR(50)::レポート種別:Report type','config:JSONB::設定:Config','generated_at:TIMESTAMP::生成日時:Generated at','data:JSONB::データ:Data'],
  // ── DevTool ──
  ApiKey:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','key_prefix:VARCHAR(20):NOT NULL:キー接頭辞:Key prefix','key_hash:TEXT:NOT NULL:キーハッシュ:Key hash','label:VARCHAR(100)::ラベル:Label','permissions:JSONB::権限:Permissions','last_used_at:TIMESTAMP::最終使用日:Last used','expires_at:TIMESTAMP::有効期限:Expires at'],
  ...
  Documentation:['title:VARCHAR(255):NOT NULL:タイトル:Title','slug:VARCHAR(255):UNIQUE:スラグ:Slug','content:TEXT:NOT NULL:内容:Content','category:VARCHAR(100)::カテゴリ:Category','sort_order:INT:DEFAULT 0:表示順:Sort order','version:VARCHAR(20)::バージョン:Version'],
  // ── Creator ──
  ...
  Tier:['user_id:UUID:FK(User) NOT NULL:クリエイターID:Creator ID','tier_name:VARCHAR(100):NOT NULL:ティア名:Tier name','price:DECIMAL(10,2):NOT NULL:月額:Monthly price','description:TEXT::説明:Description','benefits:JSONB::特典:Benefits'],
  // ── Newsletter ──
  Subscriber:['email:VARCHAR(255):UNIQUE NOT NULL:メールアドレス:Email','user_id:UUID:FK(User):ユーザーID:User ID','status:VARCHAR(20):DEFAULT \'active\':購読状態:Status','subscribed_at:TIMESTAMP:DEFAULT NOW:購読開始日:Subscribed at','tags:JSONB::タグ:Tags'],
  Campaign:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','title:VARCHAR(255):NOT NULL:キャンペーン名:Campaign title','subject:VARCHAR(255)::件名:Subject','body:TEXT::本文:Body','status:VARCHAR(20):DEFAULT \'draft\':ステータス:Status','scheduled_at:TIMESTAMP::配信予定:Scheduled at','sent_at:TIMESTAMP::配信日時:Sent at'],
  ...
  Plan:['plan_name:VARCHAR(100):NOT NULL:プラン名:Plan name','price:DECIMAL(10,2):NOT NULL:価格:Price','interval:VARCHAR(20):DEFAULT \'monthly\':課金間隔:Billing interval','features:JSONB::機能:Features','stripe_price_id:TEXT::Stripe価格ID:Stripe price ID','is_active:BOOLEAN:DEFAULT true:有効:Active'],
  // ── PWA ──
  SyncQueue:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','action:VARCHAR(50):NOT NULL:操作:Action','payload:JSONB:NOT NULL:データ:Payload','status:VARCHAR(20):DEFAULT \'pending\':同期状態:Sync status','retry_count:INT:DEFAULT 0:リトライ数:Retry count'],
  Setting:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','key:VARCHAR(100):NOT NULL:設定キー:Setting key','value:JSONB::設定値:Value'],
  // ── Booking ──
  Staff:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','specialty:VARCHAR(100)::専門分野:Specialty','is_available:BOOLEAN:DEFAULT true:利用可能:Available','calendar_config:JSONB::カレンダー設定:Calendar config'],
  ...
  // ── Event ──
  Event:['user_id:UUID:FK(User) NOT NULL:主催者ID:Organizer ID','title:VARCHAR(255):NOT NULL:イベント名:Event title','description:TEXT::説明:Description','venue_id:UUID:FK(Venue):会場ID:Venue ID','starts_at:TIMESTAMP:NOT NULL:開始日時:Starts at','ends_at:TIMESTAMP:NOT NULL:終了日時:Ends at','capacity:INT::定員:Capacity','status:VARCHAR(20):DEFAULT \'draft\':ステータス:Status','is_online:BOOLEAN:DEFAULT false:オンライン:Online'],
  ...
  Attendee:['event_id:UUID:FK(Event) NOT NULL:イベントID:Event ID','user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','ticket_id:UUID:FK(Ticket):チケットID:Ticket ID','status:VARCHAR(20):DEFAULT \'registered\':参加状態:Status','checked_in_at:TIMESTAMP::チェックイン日時:Checked in at'],
  ...
  Session:['event_id:UUID:FK(Event) NOT NULL:イベントID:Event ID','title:VARCHAR(255):NOT NULL:セッション名:Session title','speaker:VARCHAR(255)::登壇者:Speaker','starts_at:TIMESTAMP:NOT NULL:開始:Starts at','ends_at:TIMESTAMP:NOT NULL:終了:Ends at','room:VARCHAR(100)::部屋:Room'],
  Survey:['event_id:UUID:FK(Event):イベントID:Event ID','title:VARCHAR(255):NOT NULL:アンケート名:Survey title','questions:JSONB:NOT NULL:質問:Questions','is_active:BOOLEAN:DEFAULT true:受付中:Active'],
  // ── Health ──
  HealthLog:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','log_type:VARCHAR(50):NOT NULL:記録種別:Log type','value:DECIMAL(10,2):NOT NULL:値:Value','unit:VARCHAR(20)::単位:Unit','logged_at:TIMESTAMP:DEFAULT NOW:記録日時:Logged at','notes:TEXT::メモ:Notes'],
  Workout:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','workout_type:VARCHAR(50):NOT NULL:運動種別:Workout type','duration_min:INT::所要時間(分):Duration(min)','calories:INT::消費カロリー:Calories','intensity:VARCHAR(20)::強度:Intensity','notes:TEXT::メモ:Notes'],
  Meal:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','meal_type:VARCHAR(20):NOT NULL:食事種別:Meal type','food_items:JSONB::食品:Food items','calories:INT::カロリー:Calories','photo_url:TEXT::写真URL:Photo URL','logged_at:TIMESTAMP:DEFAULT NOW:記録日時:Logged at'],
  Goal:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','goal_type:VARCHAR(50):NOT NULL:目標種別:Goal type','target_value:DECIMAL(10,2):NOT NULL:目標値:Target value','current_value:DECIMAL(10,2):DEFAULT 0:現在値:Current value','unit:VARCHAR(20)::単位:Unit','deadline:DATE::期限:Deadline','status:VARCHAR(20):DEFAULT \'active\':ステータス:Status'],
  // ── HR ──
  JobPosting:['department_id:UUID:FK(Department):部署ID:Department ID','title:VARCHAR(255):NOT NULL:求人タイトル:Job title','description:TEXT::説明:Description','employment_type:VARCHAR(50)::雇用形態:Employment type','salary_range:VARCHAR(100)::給与レンジ:Salary range','status:VARCHAR(20):DEFAULT \'open\':募集状態:Status','closes_at:DATE::締切日:Closes at'],
  Applicant:['job_id:UUID:FK(JobPosting) NOT NULL:求人ID:Job ID','user_id:UUID:FK(User):ユーザーID:User ID','applicant_name:VARCHAR(255):NOT NULL:氏名:Name','email:VARCHAR(255):NOT NULL:メール:Email','resume_url:TEXT::履歴書URL:Resume URL','status:VARCHAR(20):DEFAULT \'applied\':選考状態:Status','applied_at:TIMESTAMP:DEFAULT NOW:応募日:Applied at'],
  Interview:['applicant_id:UUID:FK(Applicant) NOT NULL:応募者ID:Applicant ID','interviewer_id:UUID:FK(User) NOT NULL:面接官ID:Interviewer ID','scheduled_at:TIMESTAMP:NOT NULL:予定日時:Scheduled at','duration_min:INT:DEFAULT 60:所要時間(分):Duration(min)','interview_type:VARCHAR(50)::面接種別:Type','status:VARCHAR(20):DEFAULT \'scheduled\':ステータス:Status','notes:TEXT::メモ:Notes'],
  ...
  Onboarding:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','checklist:JSONB::チェックリスト:Checklist','progress:INT:DEFAULT 0:進捗(%):Progress(%)','mentor_id:UUID:FK(User):メンターID:Mentor ID','start_date:DATE::開始日:Start date','status:VARCHAR(20):DEFAULT \'in_progress\':ステータス:Status'],
  // ── Link in Bio ──
  Page:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','slug:VARCHAR(100):UNIQUE NOT NULL:スラグ:Slug','title:VARCHAR(255)::タイトル:Title','bio:TEXT::自己紹介:Bio','theme_id:UUID:FK(Theme):テーマID:Theme ID','is_published:BOOLEAN:DEFAULT true:公開:Published'],
  Link:['page_id:UUID:FK(Page) NOT NULL:ページID:Page ID','title:VARCHAR(255):NOT NULL:リンク名:Link title','url:TEXT:NOT NULL:URL:URL','sort_order:INT:DEFAULT 0:表示順:Sort order','is_active:BOOLEAN:DEFAULT true:有効:Active','click_count:INT:DEFAULT 0:クリック数:Clicks'],
  ...
  Integration:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','service:VARCHAR(100):NOT NULL:サービス名:Service','access_token:TEXT::アクセストークン:Access token','config:JSONB::設定:Config','status:VARCHAR(20):DEFAULT \'active\':ステータス:Status'],
  // ── Gamification ──
  Badge:['badge_name:VARCHAR(100):NOT NULL:バッジ名:Badge name','description:TEXT::説明:Description','icon_url:TEXT::アイコンURL:Icon URL','criteria:JSONB::獲得条件:Criteria','points:INT:DEFAULT 0:ポイント:Points'],
  Challenge:['title:VARCHAR(255):NOT NULL:チャレンジ名:Challenge title','description:TEXT::説明:Description','challenge_type:VARCHAR(50)::種別:Type','reward_points:INT:DEFAULT 0:報酬ポイント:Reward points','starts_at:TIMESTAMP::開始日:Starts at','ends_at:TIMESTAMP::終了日:Ends at','status:VARCHAR(20):DEFAULT \'active\':ステータス:Status'],
  Reward:['reward_name:VARCHAR(255):NOT NULL:報酬名:Reward name','description:TEXT::説明:Description','cost_points:INT:NOT NULL:必要ポイント:Cost points','stock:INT:DEFAULT -1:在庫(-1=無制限):Stock','reward_type:VARCHAR(50)::報酬種別:Type'],
  Leaderboard:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','score:INT:DEFAULT 0:スコア:Score','rank:INT::順位:Rank','period:VARCHAR(20):DEFAULT \'all_time\':期間:Period'],
  PointLog:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','points:INT:NOT NULL:ポイント:Points','action:VARCHAR(100):NOT NULL:操作:Action','balance_after:INT::残高:Balance after'],
  Achievement:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','badge_id:UUID:FK(Badge) NOT NULL:バッジID:Badge ID','earned_at:TIMESTAMP:DEFAULT NOW:獲得日時:Earned at'],
  // ── Collab ──
  Document:['workspace_id:UUID:FK(Workspace):ワークスペースID:Workspace ID','owner_id:UUID:FK(User) NOT NULL:オーナーID:Owner ID','title:VARCHAR(255):NOT NULL:タイトル:Title','content:TEXT::内容:Content','doc_type:VARCHAR(50):DEFAULT \'text\':ドキュメント種別:Document type','is_public:BOOLEAN:DEFAULT false:公開:Public'],
  ...
  Permission:['document_id:UUID:FK(Document) NOT NULL:ドキュメントID:Document ID','user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','role:VARCHAR(20):DEFAULT \'viewer\':権限:Role','granted_at:TIMESTAMP:DEFAULT NOW:付与日時:Granted at'],
  // ── IoT ──
  ...
  Command:['device_id:UUID:FK(Device) NOT NULL:デバイスID:Device ID','command_type:VARCHAR(50):NOT NULL:コマンド種別:Command type','payload:JSONB::ペイロード:Payload','status:VARCHAR(20):DEFAULT \'pending\':ステータス:Status','executed_at:TIMESTAMP::実行日時:Executed at'],
  DeviceGroup:['group_name:VARCHAR(255):NOT NULL:グループ名:Group name','description:TEXT::説明:Description','user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID'],
  // ── Community ──
  Group:['owner_id:UUID:FK(User) NOT NULL:オーナーID:Owner ID','group_name:VARCHAR(255):NOT NULL:グループ名:Group name','description:TEXT::説明:Description','is_private:BOOLEAN:DEFAULT false:非公開:Private','member_count:INT:DEFAULT 0:メンバー数:Member count'],
  // Webhook (devtool)
  Webhook:['user_id:UUID:FK(User) NOT NULL:ユーザーID:User ID','url:TEXT:NOT NULL:Webhook URL:Webhook URL','events:JSONB:NOT NULL:イベント:Events','secret:TEXT::シークレット:Secret','is_active:BOOLEAN:DEFAULT true:有効:Active','last_triggered_at:TIMESTAMP::最終実行:Last triggered'],
};
