...
function genPillar17_PromptGenome(a, pn) {
  ...
  // v9.23 Pro: prompt evaluation harness design (ADD-only)
  var _lv17 = S.skillLv != null ? S.skillLv : (S.skill === 'beginner' ? 1 : S.skill === 'pro' ? 5 : 3);
  if (_lv17 >= 5) {
    var ph = '\n---\n\n## ' + (G ? '🎓 Pro: プロンプト評価ハーネス設計' : '🎓 Pro: Prompt Evaluation Harness Design') + '\n\n';
    ph += (G
      ? 'プロンプトの「良くなった気がする」を排除し、変更を評価データセットで検証します。\n\n### 3層評価アーキテクチャ\n\n| 層 | 手法 | コスト | 用途 |\n|----|------|--------|------|\n| L1 決定的 | 正規表現・JSON Schema・文字数 | ほぼ0 | 形式検証 (毎コミット) |\n| L2 統計的 | embedding類似度・ROUGE/BLEU | 低 | 期待出力との近さ (毎PR) |\n| L3 LLM判定 | LLM-as-Judge (ルーブリック採点) | 高 | 品質・トーン・正確性 (日次/リリース前) |\n\n### 評価データセットの作り方\n\n- **ゴールデンセット**: 代表入力20-50件 + 期待出力。本番ログから「うまくいった例」を昇格させて成長させる\n- **エッジセット**: 空入力・超長文・多言語・攻撃文字列。失敗事例が出るたびに追加 (回帰防止)\n- **判定プロンプトの検証**: LLM-Judgeそのものを人間の採点30件と突き合わせ、一致率80%以上を確認してから運用\n\n```yaml\n# promptfoo評価例\nprompts: [prompts/summarize_v2.txt]\nproviders: [anthropic:claude-sonnet-4-5]\ntests:\n  - vars: { input: file://eval/golden/case01.txt }\n    assert:\n      - type: is-json\n      - type: llm-rubric\n        value: 要約は3文以内で、元文にない事実を含まない\n```\n\n- 合格ライン: ゴールデン95%+ / エッジ80%+。下回るプロンプト変更はマージ不可 (docs/69のCIと接続)\n'
      : 'Kill "it feels better" — validate every prompt change against an evaluation dataset.\n\n### 3-Layer Evaluation Architecture\n\n| Layer | Method | Cost | Use |\n|-------|--------|------|-----|\n| L1 Deterministic | Regex, JSON Schema, length | ~0 | Format checks (every commit) |\n| L2 Statistical | Embedding similarity, ROUGE/BLEU | Low | Closeness to expected output (every PR) |\n| L3 LLM judge | LLM-as-Judge with rubric | High | Quality, tone, accuracy (nightly / pre-release) |\n\n### Building the Evaluation Dataset\n\n- **Golden set**: 20-50 representative inputs + expected outputs. Grow it by promoting successful production examples\n- **Edge set**: empty input, very long text, multilingual, attack strings. Append every real failure (regression shield)\n- **Validate the judge itself**: compare LLM-Judge against 30 human-scored samples; require ≥80% agreement before trusting it\n\n```yaml\n# promptfoo example\nprompts: [prompts/summarize_v2.txt]\nproviders: [anthropic:claude-sonnet-4-5]\ntests:\n  - vars: { input: file://eval/golden/case01.txt }\n    assert:\n      - type: is-json\n      - type: llm-rubric\n        value: Summary is ≤3 sentences and adds no facts absent from the source\n```\n\n- Pass bar: golden ≥95%, edge ≥80%. Prompt changes below the bar don\'t merge (wire into the docs/69 CI)\n');
    S.files['docs/65_prompt_genome.md'] += ph;
  }
}
