...
function genPillar1_SDD(a,pn){
  ...
  const specLines=[
    ...
    '- '+(G?'同時接続':'Concurrent Users')+': '+(S.skill==='pro'?'1000+':S.skill==='intermediate'?(G?'〜200':'~200'):(G?'〜50':'~50')),
  ...
  S.files['.spec/technical-plan.md']=[
    ...
    (function(){
      ...
      return [
        ...
        G?'    """論理削除対応 Mixin — 全エンティティに継承させる"""':'    """Soft delete Mixin — inherited by all entities"""',
        '    @declared_attr',
        '    def deleted_at(cls):',
        ...
        '',
        G?'# Repository Base — アクティブレコードのみ取得':'# Repository Base — fetch active records only',
        ...
        G?'        """論理削除済みを除外したクエリ (デフォルト)"""':'        """Query excluding soft-deleted records (default)"""',
    ...
    (function(){
      ...
      return [
        ...
        G?'    """即時 job_id を返し、バックグラウンドでエクスポートを開始"""':'    """Return job_id immediately and start the export in the background"""',
        '    job_id = str(uuid4())',
        ...
        G?'- Redis: `redis:7-alpine` を docker-compose.yml に追加':'- Redis: add `redis:7-alpine` to docker-compose.yml',
  ...
  S.files['.spec/tasks.md']=[
    ...
    '> Generated by DevForge v9 — '+date,
    '> '+(G?'計画期間':'Timeline')+': '+sched.totalWeeks+' '+(G?'週間':'weeks')+(G?' ('+sched.startDate+' 〜)':' (from '+sched.startDate+')'),'',
    _taskSummary,
