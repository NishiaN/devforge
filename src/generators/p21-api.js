...
function gen83(a,pn,G){
  ...
  doc+='---\n\n';
  doc+='## '+(G?'📐 APIスタイル選定フロー':'📐 API Style Selection Decision Flow')+'\n\n';
  doc+=(G
    ?'> 要件に基づいて最適なAPIスタイルを選定するための判断フローです。\n\n'
    :'> Decision flow for selecting the optimal API style based on requirements.\n\n');
  doc+='```\n';
  doc+=(G
    ?'リアルタイム更新が必要？\n  └─ YES → WebSocket (WS)\n  └─ NO → 内部サービス間通信？\n       └─ YES → gRPC (Protocol Buffers)\n       └─ NO → 複雑な関係クエリが必要？\n            └─ YES → GraphQL\n            └─ NO → REST (デフォルト推奨)\n'
    :'Real-time updates required?\n  └─ YES → WebSocket (WS)\n  └─ NO → Internal service-to-service?\n       └─ YES → gRPC (Protocol Buffers)\n       └─ NO → Complex relational queries needed?\n            └─ YES → GraphQL\n            └─ NO → REST (default recommendation)\n');
  doc+='```\n\n';
  doc+='### '+(G?'選定基準サマリー':'Selection Criteria Summary')+'\n\n';
  doc+='| '+(G?'スタイル':'Style')+' | '+(G?'適用ケース':'Best For')+' | '+(G?'オーバーヘッド':'Overhead')+' | '+(G?'型安全':'Type Safety')+'|\n';
  doc+='|---|---|---|---|\n';
  doc+='| REST | '+(G?'公開API・標準CRUD':'Public API, standard CRUD')+' | '+(G?'低':'Low')+' | OpenAPI |\n';
  doc+='| GraphQL | '+(G?'複数リソース結合・FE主導':'Multi-resource, FE-driven')+' | '+(G?'中':'Medium')+' | Schema |\n';
  doc+='| gRPC | '+(G?'マイクロサービス間・高性能':'Microservices, high performance')+' | '+(G?'低':'Low')+' | Protobuf |\n';
  doc+='| WebSocket | '+(G?'リアルタイム・双方向':'Real-time, bidirectional')+' | '+(G?'中':'Medium')+' | — |\n\n';
