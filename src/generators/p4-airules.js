...
`;
  ...
  const _beFull=arch.isBaaS?(a.backend||'Firebase')+' Functions, RLS policies':arch.pattern==='bff'?'Next.js API Routes, middleware':'API routes, database, auth';
  S.files['.claude/agents/requirements-agent.md']='---\nname: requirements-agent\ndescription: '+( G?'要件定義エージェント。仕様の曖昧さ検出・受入基準生成・仕様穴特定を行う':'Requirements agent. Detects spec ambiguity, generates acceptance criteria, identifies spec holes')+'\ntools: Read, Glob, Grep, Write\n---\n\n# '+(G?'要件定義エージェント':'Requirements Agent')+' — '+pn+'\n\n## '+(G?'責務':'Responsibilities')+'\n- '+( G?'.spec/constitution.md の使命・KPIと仕様書の整合性検証':'Verify alignment between .spec/constitution.md and requirements')+'\n- '+( G?'「適切に」「柔軟に」等の非定量表現を検出して定量化提案':'Detect non-quantified terms and propose quantification')+'\n- '+( G?'受入基準 (Given/When/Then) の生成':'Generate acceptance criteria (Given/When/Then)')+'\n- '+( G?'仕様穴 P0/P1/P2 の特定と修正案提示':'Identify spec holes P0/P1/P2 and propose fixes')+'\n\n## '+(G?'入力':'Inputs')+'\n- `.spec/constitution.md` — '+(G?'使命・KPI・設計原則':'Mission, KPI, design principles')+'\n- `docs/13_glossary.md` — '+(G?'ユビキタス言語辞書':'Ubiquitous language')+'\n- `docs/114_domain_knowledge_guide.md` — '+(G?'業務ルール・ワークフロー':'Business rules, workflows')+'\n\n## '+(G?'出力':'Outputs')+'\n- `.spec/specification.md` — '+(G?'受入基準付き要件定義書の更新':'Updated requirements with acceptance criteria')+'\n\n## '+(G?'判断基準':'Judgment Criteria')+'\n- '+(G?'非定量表現ゼロ（全て定量値あり）':'Zero non-quantified expressions (all have quantified values)')+'\n- '+(G?'全要件に受入基準（Given/When/Then）付与':'All requirements have AC (Given/When/Then)')+'\n- '+(G?'P0仕様穴ゼロ（実装開始前に解決）':'Zero P0 spec holes (resolved before implementation starts)')+'\n\n## '+(G?'アーキテクチャ固有指示':'Architecture-specific Instructions')+'\n'+(arch.isBaaS?'- '+(G?'BaaSのRLSポリシーを要件として明示する':'Explicitly state BaaS RLS policies as requirements'):arch.pattern==='bff'?'- '+(G?'BFFパターンのAPI境界を要件として定義する':'Define BFF API boundaries as requirements'):'- '+(G?'REST/GraphQL API設計の制約を要件に含める':'Include REST/GraphQL API design constraints in requirements')+'\n')+'- '+(G?'docs/13_glossary.mdのユビキタス言語を要件定義全体で統一使用':'Use ubiquitous language from docs/13_glossary.md throughout requirements')+'\n';

  S.files['.claude/agents/design-agent.md']='---\nname: design-agent\ndescription: '+( G?'設計エージェント。アーキテクチャ・ER図・API設計の整合性を検証し技術的決定を行う':'Design agent. Verifies architecture/ER/API consistency and makes technical decisions')+'\ntools: Read, Glob, Grep, Write\n---\n\n# '+(G?'設計エージェント':'Design Agent')+' — '+pn+'\n\n## '+(G?'責務':'Responsibilities')+'\n- '+( G?'docs/03_architecture.md と実装の乖離を検出':'Detect drift between docs/03_architecture.md and implementation')+'\n- '+( G?'ER図のエンティティ整合性（FK・正規化・命名規則）確認':'Verify ER entity consistency (FK, normalization, naming)')+'\n- '+( G?'ADR（アーキテクチャ決定記録）の作成と更新':'Create and update ADRs (Architecture Decision Records)')+'\n- '+( G?'.spec/technical-plan.md の技術的決定の妥当性評価':'Evaluate technical decision validity in .spec/technical-plan.md')+'\n\n## '+(G?'入力':'Inputs')+'\n- `.spec/specification.md`\n- `docs/03_architecture.md`\n- `docs/04_er_diagram.md`\n- `docs/13_glossary.md`\n\n## '+(G?'出力':'Outputs')+'\n- `.spec/technical-plan.md` — '+(G?'更新':'updated')+'\n- `docs/00_architecture_decision_records.md` — '+(G?'ADR追記':'ADR appended')+'\n\n## '+(G?'判断基準':'Judgment Criteria')+'\n- '+(G?'アーキテクチャと実装の乖離ゼロ':'Zero drift between architecture and implementation')+'\n- '+(G?'ER図のFK全て解決済み（未定義エンティティ参照なし）':'All ER FK references resolved (no undefined entity references)')+'\n- '+(G?'重要決定は全てADRに記録':'All significant decisions recorded in ADR')+'\n\n## '+(G?'アーキテクチャ固有指示':'Architecture-specific Instructions')+'\n'+(arch.isBaaS?'- '+(G?'RLSポリシーはERDと同時に設計する':'Design RLS policies alongside ERD'):arch.pattern==='bff'?'- '+(G?'BFF層のデータフェッチ戦略を明確化する':'Clarify BFF data fetching strategy'):'- '+(G?'APIのバージョニング戦略をADRに記録する':'Record API versioning strategy in ADR'))+'\n';

  S.files['.claude/agents/task-planner-agent.md']='---\nname: task-planner-agent\ndescription: '+( G?'タスク計画エージェント。仕様書からP0/P1/P2優先度付きタスクを生成し依存関係を解析する':'Task planner agent. Generates P0/P1/P2 prioritized tasks from specs and analyzes dependencies')+'\ntools: Read, Glob, Write\n---\n\n# '+(G?'タスク計画エージェント':'Task Planner Agent')+' — '+pn+'\n\n## '+(G?'責務':'Responsibilities')+'\n- '+( G?'仕様書からエピック→ストーリー→タスクへの分解':'Break down specs into Epic→Story→Task hierarchy')+'\n- '+( G?'P0（KPI直結）/ P1（品質影響）/ P2（推奨）の優先度付与':'Assign P0 (KPI-critical) / P1 (quality-impact) / P2 (recommended) priorities')+'\n- '+( G?'タスク間依存関係の特定（ブロッカー/ブロックされる）':'Identify inter-task dependencies (blocker/blocked-by)')+'\n- '+( G?'完了定義（DoD）の設定':'Set Done of Definition (DoD) per task')+'\n\n## '+(G?'入力':'Inputs')+'\n- `.spec/specification.md`\n- `.spec/tasks.md` — '+(G?'既存タスク（更新ベース）':'existing tasks (basis for update)')+'\n- `docs/13_glossary.md`\n\n## '+(G?'出力':'Outputs')+'\n- `.spec/tasks.md` — '+(G?'優先度・受入基準・見積時間付きタスク更新':'Updated tasks with priority, AC, and time estimates')+'\n\n## '+(G?'判断基準':'Judgment Criteria')+'\n- '+(G?'全タスクに優先度ラベル（P0/P1/P2）が付いている':'All tasks have priority labels (P0/P1/P2)')+'\n- '+(G?'依存関係が循環参照なしに解決されている':'Dependencies resolved without circular references')+'\n- '+(G?'Sprint 1 にP0タスクが集中している':'P0 tasks concentrated in Sprint 1')+'\n\n## '+(G?'タスク見積ヒューリスティック':'Task Estimation Heuristics')+'\n- '+(G?'インフラ・環境構築: 2h':'Infrastructure setup: 2h')+'\n- '+(G?'機能実装（1機能）: 4-8h':'Feature implementation (1 feature): 4-8h')+'\n- '+(G?'画面実装（1画面）: 4h':'Screen implementation (1 screen): 4h')+'\n- '+(G?'テスト作成（1機能）: 4h':'Test creation (1 feature): 4h')+'\n';

  S.files['.claude/agents/implementation-agent.md']='---\nname: implementation-agent\ndescription: '+( G?'実装エージェント。仕様書・技術計画・APIドキュメントに忠実にコードを生成する':'Implementation agent. Generates code faithful to specs, technical plans, and API docs')+'\ntools: Read, Glob, Grep, Write, Bash\n---\n\n# '+(G?'実装エージェント':'Implementation Agent')+' — '+pn+'\n\n## '+(G?'責務':'Responsibilities')+'\n- '+( G?'.spec/tasks.md のP0タスクから順に実装':'Implement starting from P0 tasks in .spec/tasks.md')+'\n- '+( G?'TypeScript型定義→データアクセス層→ビジネスロジック→UIの順序で実装':'Implement in order: TypeScript types → data access → business logic → UI')+'\n- '+( G?'各機能に対応するVitestユニットテストを同時作成':'Create Vitest unit tests alongside each feature')+'\n- '+( G?'docs/13_glossary.md の用語を変数名・コメントに使用':'Use docs/13_glossary.md terms in variable names and comments')+'\n\n## '+(G?'入力':'Inputs')+'\n- `.spec/technical-plan.md`\n- `docs/05_api_design.md`\n- `CLAUDE.md` — '+(G?'コーディング規約':'Coding conventions')+'\n- `docs/13_glossary.md`\n\n## '+(G?'出力':'Outputs')+'\n- `src/` — '+(G?'TypeScriptソースコード':'TypeScript source code')+'\n- `tests/` — '+(G?'Vitestユニットテスト':'Vitest unit tests')+'\n\n## '+(G?'アーキテクチャ固有指示':'Architecture-specific Instructions')+'\n'+(arch.isBaaS?
    '- '+( G?'DB操作は'+a.backend+' SDK経由のみ（直接SQLは禁止）':'DB operations only via '+a.backend+' SDK (direct SQL prohibited)')+'\n- '+( G?'RLSポリシーと整合するクライアント側アクセス制御を実装':'Implement client-side access control consistent with RLS policies'):
    arch.pattern==='bff'?
    '- '+( G?'API RoutesでBFF層を実装し、フロントエンドに直接DBアクセスを持たせない':'Implement BFF layer in API Routes; frontend must not have direct DB access'):
    '- '+( G?'APIエンドポイントはdocs/05_api_design.mdのOpenAPI仕様に完全準拠':'API endpoints must fully comply with OpenAPI spec in docs/05_api_design.md')
  )+'\n- '+(G?'docs/08_security.mdのセキュリティチェックリストを実装時に確認':'Check docs/08_security.md security checklist during implementation')+'\n';
...
`;
  ...
  if(aiAuto&&!isNone(aiAuto)){
    ...
    if(isMultiOrAbove){
      ...
      specHoleMd+=(G?'## 実行手順':'## Execution Steps')+'\n\n';
      specHoleMd+='1. '+(G?'各観点でspec読み込み (曖昧/正常系のみ/権限/バリデ/エラー/監査/運用/性能/セキュリティ)':'Read spec for each of 9 perspectives (ambiguity/happy-path/auth/validation/error/audit/ops/perf/security)')+'\n';
      specHoleMd+='2. '+(G?'穴をP0/P1/P2で分類':'Classify holes as P0/P1/P2')+'\n';
      specHoleMd+='3. '+(G?'P0穴ごとに最短修正案を1文で提示':'Propose 1-sentence fix for each P0 hole')+'\n\n';
      specHoleMd+=(G?'## 出力フォーマット':'## Output Format')+'\n\n';
      specHoleMd+='```\n';
      specHoleMd+=(G?'## 穴あき検出結果':'## Spec Hole Report')+'\n';
      specHoleMd+='| # | '+(G?'観点':'Perspective')+' | '+(G?'穴の内容':'Hole Description')+' | '+(G?'分類':'Class')+' | '+(G?'修正案 (P0のみ)':'Fix Proposal (P0 only)')+' |\n';
      specHoleMd+='|---|--------|---------|------|-------|\n';
      specHoleMd+='| 1 | '+(G?'権限抜け':'Auth gap')+' | admin '+(G?'の DELETE 権限が未定義':'DELETE permission undefined')+' | P0 | '+(G?'"管理者のみDELETE可" をspec追記':'Add "admin-only DELETE" to spec')+' |\n';
      specHoleMd+='```\n\n';
      specHoleMd+=(G?'## 判断基準':'## Judgment Criteria')+'\n\n';
      specHoleMd+='- '+_priorityLabel('P0',G)+': '+( G?'実装停止・仕様確定必須':'Stop impl — must confirm spec')+'\n';
      specHoleMd+='- '+_priorityLabel('P1',G)+': '+(G?'実装前に修正推奨':'Recommend fix before impl')+'\n';
      specHoleMd+='- '+_priorityLabel('P2',G)+': '+(G?'任意 (Issue 登録)':'Optional (log as Issue)')+'\n';
      S.files['skills/spec-hole-detect.md']=specHoleMd;
    }
