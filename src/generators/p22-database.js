...
function gen87(a,pn,G){
  ...
  // Domain-specific DB hardening rules
  var _db87dom=typeof detectDomain==='function'?detectDomain(a.purpose||''):null;
  var _db87ops=typeof DOMAIN_OPS!=='undefined'&&_db87dom?(DOMAIN_OPS[_db87dom]||null):null;
  if(_db87ops&&_db87ops.hardening_ja&&_db87ops.hardening_ja.length>0){
    doc+='## '+(G?'ドメイン固有DBハードニングルール ('+_db87dom+')':'Domain-Specific DB Hardening ('+_db87dom+')')+'\n\n';
    var _h87=G?_db87ops.hardening_ja:_db87ops.hardening_en;
    _h87.forEach(function(h){doc+='- ✅ '+h+'\n';});
    doc+='\n> '+(G?'これらのルールは '+_db87dom+' ドメインの規制・リスク要件から自動導出されています。':'These rules are auto-derived from regulatory/risk requirements of the '+_db87dom+' domain.')+'\n\n';
  }

  // Entity-specific CREATE TABLE examples
  var _p87ents=(a.data_entities||'').split(',').map(function(e){return e.trim();}).filter(Boolean);
  if(_p87ents.length&&!isMongo&&!isBaaS){
    doc+='## '+(G?'エンティティ別テーブル定義例':'Entity Table Definitions')+'\n\n';
    doc+=(G
      ?'> プロジェクトのエンティティ定義に基づく参照スキーマです。マイグレーションファイルとして使用できます。\n\n'
      :'> Reference schema based on your project entities. Can be used as migration files.\n\n'
    );
    _p87ents.slice(0,3).forEach(function(ent){
      var cols=getEntityColumns(ent,G,_p87ents);
      if(!cols.length)return;
      var tbl=ent.replace(/([A-Z])/g,function(m,c,i){return (i>0?'_':'')+c.toLowerCase();});
      doc+='### `'+tbl+'`\n\n```sql\nCREATE TABLE '+tbl+' (\n';
      doc+='  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),\n';
      cols.forEach(function(c){
        if(!c)return;
        var cn=c.col,ct=c.type||'TEXT',cv=c.constraint||'',lbl=c.desc||cn;
        if(/^(id|created_at|updated_at|deleted_at)$/.test(cn))return;
        doc+='  '+cn+' '+ct+(cv?' '+cv:'')+',  -- '+lbl+'\n';
      });
      doc+='  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),\n';
      doc+='  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),\n';
  ...
  if((a.scale||'medium')!=='solo'){
    doc+='---\n\n';
    doc+='## '+(G?'📊 レプリケーション & シャーディングパターン':'📊 Replication & Sharding Patterns')+'\n\n';
    doc+=(G
      ?'> スケール要件に基づいてデータベース拡張パターンを選定してください。\n\n'
      :'> Select database scaling pattern based on your scale requirements.\n\n');
    doc+='### '+(G?'パターン選定マトリクス':'Pattern Selection Matrix')+'\n\n';
    doc+='| '+(G?'負荷パターン':'Load Pattern')+' | '+(G?'推奨アーキテクチャ':'Recommended Architecture')+' | '+(G?'適用例':'Use Case')+'|\n';
    doc+='|---|---|---|\n';
    doc+='| '+(G?'読取重視 (>80% READ)':'Read-heavy (>80% READ)')+' | '+(G?'Read Replica (Primary + N Replica)':'Read Replica (Primary + N Replicas)')+' | '+(G?'レポート・検索・ダッシュボード':'Reports, search, dashboards')+'|\n';
    doc+='| '+(G?'書込重視 (>50% WRITE)':'Write-heavy (>50% WRITE)')+' | '+(G?'水平シャーディング (Hash/Range)':'Horizontal Sharding (Hash/Range)')+' | '+(G?'ログ収集・IoT・トランザクション':'Log collection, IoT, transactions')+'|\n';
    doc+='| '+(G?'両方大規模':'Both at scale')+' | '+(G?'シャーディング + Replica':'Sharding + Replicas')+' | '+(G?'大規模SaaS・マーケットプレイス':'Large SaaS, marketplaces')+'|\n\n';
    doc+='### '+(G?'実装チェックリスト':'Implementation Checklist')+'\n\n';
    doc+=(G?'**Read Replica:**\n':'**Read Replica:**\n');
    doc+='- [ ] '+(G?'Prisma: `datasource db { url = env("DATABASE_URL") readUrl = env("DATABASE_URL_REPLICA") }`':'Prisma: `datasource db { url = env("DATABASE_URL") readUrl = env("DATABASE_URL_REPLICA") }`')+'\n';
    doc+='- [ ] '+(G?'SELECT系クエリをReplicaへルーティング':'Route SELECT queries to replica')+'\n';
    doc+='- [ ] '+(G?'レプリケーション遅延の許容値設定 (<500ms)':'Set replication lag tolerance (<500ms)')+'\n';
    doc+='- [ ] '+(G?'Replicaフェイルオーバー時のPrimaryフォールバック実装':'Implement primary fallback on replica failover')+'\n\n';
    doc+=(G?'**シャーディング:**\n':'**Sharding:**\n');
    doc+='- [ ] '+(G?'シャードキー選定 (Cardinality高・ホットスポット回避)':'Shard key selection (high cardinality, avoid hotspots)')+'\n';
    doc+='- [ ] '+(G?'クロスシャードクエリ最小化':'Minimize cross-shard queries')+'\n';
    doc+='- [ ] '+(G?'シャード間データ整合性戦略 (SAGA / 2PC)':'Cross-shard consistency strategy (SAGA / 2PC)')+'\n\n';
    doc+='### '+(G?'監視メトリクス':'Monitoring Metrics')+'\n\n';
    doc+='| '+(G?'メトリクス':'Metric')+' | '+(G?'閾値':'Threshold')+' | '+(G?'アラート条件':'Alert Condition')+'|\n';
    doc+='|---|---|---|\n';
    doc+='| '+(G?'レプリケーション遅延':'Replication lag')+' | < 500ms | > 1s |\n';
    doc+='| '+(G?'接続プール使用率':'Connection pool usage')+' | < 80% | > 90% |\n';
    doc+='| '+(G?'スロークエリ率':'Slow query rate')+' | < 1% | > 5% |\n\n';
  }
...
function gen90(a,pn,G){
  ...
  // Domain-specific backup/recovery requirements from DOMAIN_OPS
  var _db90dom=typeof detectDomain==='function'?detectDomain(a.purpose||''):null;
  var _db90ops=typeof DOMAIN_OPS!=='undefined'&&_db90dom?(DOMAIN_OPS[_db90dom]||null):null;
  if(_db90ops&&_db90ops.backup_ja&&_db90ops.backup_ja.length>0){
    doc+='## '+(G?'ドメイン標準バックアップ要件 ('+_db90dom+')':'Domain-Specific Backup Requirements ('+_db90dom+')')+'\n\n';
    doc+='> '+(G?'このドメインの規制・リスク要件に基づく最低限のバックアップ基準:':'Minimum backup standards based on regulatory/risk requirements for this domain:')+'\n\n';
    var _bk90=G?_db90ops.backup_ja:_db90ops.backup_en;
    _bk90.forEach(function(b){doc+='- ✅ '+b+'\n';});
    doc+='\n';
  }
