...
function applyLang(){
  ...
  const pbJa=['①SDD統合','②DevContainer','③MCP設定','④AIルール','⑤並列探索','⑥Dashboard','⑦ロードマップ','⑧AIランチャー','⑨デザインシステム','⑩リバースEng','⑪実装ガイド','⑫セキュリティ','⑬戦略インテリジェンス','⑭運用インテリジェンス','⑮未来戦略','⑯開発IQ','⑰プロンプトゲノム','⑱Prompt Ops','⑲エンタープライズ','⑳CI/CD','㉑APIインテリジェンス','㉒DBインテリジェンス','㉓テストインテリジェンス','㉔AI安全性'];
  const pbEn=['①SDD','②DevContainer','③MCP','④AI Rules','⑤Explorer','⑥Dashboard','⑦Roadmap','⑧AI Launcher','⑨Design System','⑩Reverse Eng','⑪Impl Guide','⑫Security','⑬Strategic Intelligence','⑭Ops Intelligence','⑮Future Strategy','⑯Dev IQ','⑰Prompt Genome','⑱Prompt Ops','⑲Enterprise','⑳CI/CD','㉑API Intelligence','㉒DB Intelligence','㉓Testing Intelligence','㉔AI Safety'];
  pbadges.forEach((el,i)=>{if(i<24)el.textContent=ja?pbJa[i]:pbEn[i];});
  ...
  pbadges.forEach((el,i)=>{if(i<24)el.title=ja?pbTipJa[i]:pbTipEn[i];});
  // Beginner badge filtering: Lv0-1 shows only 4 essential pillars to reduce cognitive load
  var _bpSet=new Set([0,3,7,8]);
  pbadges.forEach(function(el,i){if(i<23){el.style.display=(S.skillLv<=1&&!_bpSet.has(i))?'none':'';}});
