...
function _getTourSteps(){
  ...
  {title:_ja?'📝 テンプレート':'📝 Templates',desc:_ja?
    '📦 標準157種 ＋ 🎓 分野別503種のプリセットを選択できます。<br>💡 「モード切替」で標準↔分野を切り替え。⚔️ 比較ボタンで2プリセットを並べて比較。<br>💡 「おすすめ」フィルターで選択肢を絞り込み。選ぶと25項目の回答が自動入力。<br>間違えて選んだ場合は Ctrl+Z（Cmd+Z）で直前のプリセット適用を取り消せます。':
    '📦 157 standard + 🎓 503 field presets available.<br>💡 Use "Mode Toggle" to switch standard ↔ field. ⚔️ Compare button shows 2 presets side-by-side.<br>💡 "Recommended" filter narrows choices. Selecting auto-fills 25 answers.<br>Changed your mind? Ctrl+Z (Cmd+Z) undoes the last preset application.'},
  ...
  {title:_ja?'📦 212+ファイル生成':'📦 212+ File Generation',desc:_ja?
    '全質問回答後、27の柱で212+ファイルを自動生成。<br>💡 生成内訳: .spec/（仕様書5点）/ docs/（108ドキュメント）/ .claude/（AIエージェント設定）/ .github/（CI/CDワークフロー）/ db/（シードデータ）など。<br>💡 ピラータブの 🔄 ボタンで個別ピラーのみ再生成可能。回答を微調整したあとの部分更新に便利。':
    'All answers collected → 212+ files auto-generate across 27 pillars.<br>💡 Breakdown: .spec/ (5 specs) / docs/ (108 docs) / .claude/ (AI agent configs) / .github/ (CI/CD) / db/ (seed data) & more.<br>💡 Click 🔄 on any pillar tab to regenerate just that pillar — perfect for partial updates after tweaking answers.'},
  ...
  {title:_ja?'🤖 AIランチャー':'🤖 AI Launcher',desc:_ja?
    'Pillar ⑧ の AIランチャーで仕様書をAIツールにワンクリック投入。<br>💡 57テンプレートをコード生成・レビュー・ドキュメント・ブレストなどのカテゴリで整理。<br>💡 あなたのスキルレベルに合わせたテンプレートを優先表示（スキル適応レコメンド）。<br>💡 トークン推定・モデル選択・出力プレビューも搭載。Claude / GPT-4o / Gemini に対応。':
    'Pillar ⑧: Feed specs to AI tools in one click from AI Launcher.<br>💡 57 templates organized by category: code-gen, review, docs, brainstorm & more.<br>💡 Templates are prioritized by your skill level (skill-adaptive recommendation).<br>💡 Token estimation, model selection, and output preview built in. Works with Claude / GPT-4o / Gemini.'},
