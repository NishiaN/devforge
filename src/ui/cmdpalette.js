...
function updateCmdResults(query,_ja,input,resultsDiv){
  ...
  const commands=[
    ...
    // Navigate group
    ...(hasFiles?[
      {cat:'navigate',icon:'📊',label:_ja?'ダッシュボード':'Dashboard',labelEn:'Dashboard',kb:'Ctrl+5',action:()=>{S.pillar=5;saveDebounced();showFileTree();}},
      {cat:'navigate',icon:'🗺️',label:_ja?'ロードマップ':'Roadmap',labelEn:'Roadmap',kb:'Ctrl+6',action:()=>{S.pillar=6;saveDebounced();showRoadmapUI();}},
