...
function showPostGenGuide(force){
  ...
  const steps=isB?(_ja?[
    ...
    ['116テンプレートパイプライン','柱⑧で📋レビュー→🔨実装→🧪テスト→♻️リファクタ→🔒セキュリティ→📝ドキュメント→🛡️Ops準備。全工程自動化。'],
    ...
    ['116-Template Pipeline','Pillar ⑧: 📋Review → 🔨Implement → 🧪Test → ♻️Refactor → 🔒Security → 📝Docs → 🛡️Ops. Full automation.'],
    ...
    ${S.skillLv===2?`<div class="guide-bridge">
      ...
      <div class="guide-bridge-cards">
        ...
        <button class="guide-bridge-card" onclick="this.closest('.guide-overlay').remove();var _t=document.querySelectorAll('.piltab');if(_t[7])_t[7].click();">
          ...
          <div class="bridge-why">${_ja?'116種プロンプトでAIをプロジェクト専用に即戦力化':'116 prompt templates — make AI your dedicated team member'}</div>
        </button>
