...
function showManual(sec){
  ...
  const _ja=S.lang==='ja';
  const MANUAL=[
    {id:'overview',title:_ja?'概要':'Overview',body:_ja?'<h2>DevForge v9.22 とは</h2><p>質問に答えるだけで、プロジェクトに必要な227+ファイルを自動生成するAI駆動開発プラットフォームです。'+_TECH_COUNT+'テクノロジー対応。</p>'+
      ...
      '<p>+ docs（134仕様書）+ 共通ファイル（4）= <strong>227+ファイル</strong></p>':'<h2>What is DevForge v9.22?</h2><p>An AI-driven dev platform that auto-generates 227+ project files just by answering questions. Supports '+_TECH_COUNT+' technologies.</p>'+
    ...
    {id:'pillars',title:_ja?'28の柱':'28 Pillars',body:_ja?
      ...
      '<p>87_database_design_principles.md (正規化・インデックス戦略・命名規則・パーティション設計) / 88_query_optimization_guide.md (クエリ最適化・N+1問題対策・実行計画分析) / 89_migration_strategy.md (ゼロダウンタイムマイグレーション・ロールバック手順・データ整合性) / 90_backup_disaster_recovery.md (RTO/RPO目標・PITRポリシー・DRランブック) — ORM別(Prisma/Drizzle/TypeORM/SQLAlchemy/Kysely)対応。</p>'+
      '<h3>㉓ テストインテリジェンス (4ファイル)</h3>'+
      '<p>91_testing_strategy.md (テストピラミッド・フレームワーク選定・TDDワークフロー) / 92_coverage_design.md (カバレッジ目標・ミューテーションテスト・モック戦略) / 93_e2e_test_architecture.md (Playwright POM・認証storageState・CI並列化・視覚回帰テスト) / 94_performance_testing.md (Web Vitals計測・Lighthouse CI・k6負荷テスト・SLO検証) — フロントエンド/バックエンド/BaaS別対応。</p>'+
      '<h3>㉔ AI安全性インテリジェンス (4ファイル)</h3>'+
      '<p>95_ai_safety_framework.md (AI機能リスク分類・脅威モデリング・ステークホルダー影響評価) / 96_ai_guardrail_implementation.md (入力/出力/コンテンツ/アクセス制御の4層ガードレール実装) / 97_ai_model_evaluation.md (精度・公平性・透明性・セキュリティ・プライバシーの評価フレームワーク) / 98_prompt_injection_defense.md (インジェクション検出・多層防御・AI固有セキュリティテスト) — Claude/OpenAI/Gemini/ローカルLLMプロバイダー別対応。</p>'+
      ...
      '<p>87_database_design_principles.md (normalization, index strategy, naming conventions, partitioning) / 88_query_optimization_guide.md (query optimization, N+1 prevention, execution plan analysis) / 89_migration_strategy.md (zero-downtime migrations, rollback procedures, data integrity) / 90_backup_disaster_recovery.md (RTO/RPO targets, PITR policies, DR runbook) — ORM-specific: Prisma/Drizzle/TypeORM/SQLAlchemy/Kysely.</p>'+
      '<h3>㉓ Test Intelligence (4 files)</h3>'+
      '<p>91_testing_strategy.md (test pyramid, framework selection, TDD workflow) / 92_coverage_design.md (coverage targets, mutation testing, mock strategy) / 93_e2e_test_architecture.md (Playwright POM, auth storageState, CI parallelization, visual regression) / 94_performance_testing.md (Web Vitals, Lighthouse CI, k6 load testing, SLO validation) — Frontend (Next.js/React/Vue/Svelte) and backend (Python/Java/Node/BaaS) specific.</p>'+
      '<h3>㉔ AI Safety Intelligence (4 files)</h3>'+
      '<p>95_ai_safety_framework.md (AI feature risk categories, threat modeling, stakeholder impact) / 96_ai_guardrail_implementation.md (4-layer guardrails: input/output/content/access control) / 97_ai_model_evaluation.md (accuracy, fairness, transparency, security, privacy evaluation) / 98_prompt_injection_defense.md (injection detection, defense-in-depth, AI-specific security testing) — Provider-specific: Claude/OpenAI/Gemini/Local LLM.</p>'+
    ...
    {id:'export',title:_ja?'エクスポート':'Export',body:_ja?
      ...
      :
      '<h2>Export Methods</h2><p><strong>ZIP</strong>: Download all 227+ files as a ZIP with folder structure.<br><strong>PDF</strong>: Format Markdown files and print via browser PDF.<br><strong>URL Sharing</strong>: Base64-encode project settings and share via URL.<br><strong>Copy All Files</strong>: Combine all documents into one text and copy to clipboard (Ctrl+Shift+C). Ideal for bulk AI input.</p><h3>Template Save</h3><p>Save project settings to localStorage and load them on next launch.</p>'+
    ...
    {id:'guide',title:_ja?'🚀 活用ガイド':'🚀 Usage Guide',body:function(){
      const baseBody=_ja?
      '<h2>🚀 生成ファイル活用ガイド</h2>'+
      '<p>DevForge v9 は世界で唯一の<strong>仕様駆動AIプロジェクトジェネレーター</strong>です。他のツールが「コード」を生成するのに対し、DevForge は「開発の知性」── 設計・環境・ルール・学習計画を227+ファイルで生成します。</p>'+
      ...
      '• <strong>生成ファイルは設計書</strong> — 227+ファイルは実行コードではなく設計ドキュメント。AIツールに投入して実コードを生成<br>'+
      ...
      '<tr><td><code>docs/</code> 134ファイル</td><td>後で参照</td><td>レビュー素材</td><td>CI/CDゲート</td></tr>'+
      ...
      '<p>DevForge v9 is the world\'s only <strong>spec-driven AI project generator</strong>. While other tools generate code, DevForge generates "development intelligence" — design, environment, rules, and learning plans through 227+ files.</p>'+
      ...
      '• <strong>Files are Design Docs</strong> — 227+ files are not executable code but design documents. Feed them to AI tools to generate real code<br>'+
      ...
      '<tr><td><code>docs/</code> 134 files</td><td>Later</td><td>Review material</td><td>CI/CD gates</td></tr>'+
    ...
    {id:'workflow',title:_ja?'📘 生成後ワークフロー':'📘 Post-Gen Workflow',body:_ja?
      ...
      '<p class="guide-workflow-intro">DevForge v9で227+ファイルを生成した後、<strong>実際に動くアプリを作るまでの一気通貫の手順</strong>を5フェーズで解説します。</p>'+
      ...
      '<tr><td>docs/</td><td>134種類の仕様書・設計書</td><td>134</td></tr>'+
      ...
      '<li>全227+ファイルを一度にAIに投入しない（.spec/から開始）</li>'+
      ...
      '<p class="guide-workflow-intro">After generating 227+ files with DevForge v9, follow this <strong>end-to-end workflow</strong> in 5 phases to build a working app.</p>'+
      ...
      '<tr><td>docs/</td><td>134 spec/design documents</td><td>134</td></tr>'+
      ...
      '<li>Don\'t feed all 227+ files to AI at once (start with .spec/)</li>'+
    ...
    {id:'caution',title:_ja?'⚠️ 注意事項':'⚠️ Cautions',body:_ja?
      ...
      '<h3>🟡 生成ファイルについて</h3>'+
      '<p>生成される227+ファイルは<strong>設計ドキュメント</strong>（SDD仕様書・DevContainer設定・AIルール等）です。npm installで即座に動くアプリケーションコードではありません。Claude Code / Cursor等のAIツールに入力して実コードを生成する運用が前提です。</p>'+
      ...
      '<p>227+ generated files are <strong>design documents</strong> (SDD specs, DevContainer configs, AI rules). They are not runnable app code. Feed them to AI tools like Claude Code / Cursor to generate actual code.</p>'+
    ...
    {id:'about',title:'About',body:'<h2>DevForge v9.22</h2><p>'+(_ja?'AI駆動開発 統合プラットフォーム':'AI-Driven Dev Platform')+'</p><p>Version 9.6.0 — 2026 Edition (Modular Architecture)</p><p>'+(_ja?''+_TECH_COUNT+'テクノロジー ・ 227+ファイル ・ 28の柱 ・ 860プリセット ・ Mermaid図 ・ プロンプトプレイブック':''+_TECH_COUNT+' technologies ・ 227+ files ・ 28 pillars ・ 860 presets ・ Mermaid diagrams ・ Prompt playbook')+'</p><p>© 2026 エンジニアリングのタネ制作委員会<br>by にしあん</p>'},
  ];
  const nav=$('helpNav');
