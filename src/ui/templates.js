...
function showManual(sec){
  ...
  const MANUAL=[
    {id:'overview',title:_ja?'概要':'Overview',body:_ja?'<h2>DevForge v9.0 とは</h2><p>質問に答えるだけで、プロジェクトに必要な86+ファイルを自動生成するAI駆動開発プラットフォームです。'+_TECH_COUNT+'テクノロジー対応。</p>'+
      ...
      '<p>+ docs（38仕様書）+ 共通ファイル（4）= <strong>86+ファイル</strong></p>':'<h2>What is DevForge v9.0?</h2><p>An AI-driven dev platform that auto-generates 86+ project files just by answering questions. Supports '+_TECH_COUNT+' technologies.</p>'+
      ...
      '<p>+ docs (38 specs) + common (4) = <strong>86+ files</strong></p>'},
