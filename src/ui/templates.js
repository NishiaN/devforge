...
function showManual(sec){
  ...
  const MANUAL=[
    {id:'overview',title:_ja?'概要':'Overview',body:_ja?'<h2>DevForge v9.6 とは</h2><p>質問に答えるだけで、プロジェクトに必要な185+ファイルを自動生成するAI駆動開発プラットフォームです。'+_TECH_COUNT+'テクノロジー対応。</p>'+
      ...
      '<p>+ docs（108仕様書）+ 共通ファイル（4）= <strong>185+ファイル</strong></p>':'<h2>What is DevForge v9.6?</h2><p>An AI-driven dev platform that auto-generates 185+ project files just by answering questions. Supports '+_TECH_COUNT+' technologies.</p>'+
      ...
      '<p>+ docs (108 specs) + common (4) = <strong>185+ files</strong></p>'},
    ...
    {id:'about',title:'About',body:'<h2>DevForge v9.6</h2><p>'+(_ja?'AI駆動開発 統合プラットフォーム':'AI-Driven Dev Platform')+'</p><p>Version 9.6.0 — 2026 Edition (Modular Architecture)</p><p>'+(_ja?''+_TECH_COUNT+'テクノロジー ・ 185+ファイル ・ 26の柱 ・ 133プリセット ・ Mermaid図 ・ プロンプトプレイブック':''+_TECH_COUNT+' technologies ・ 185+ files ・ 26 pillars ・ 133 presets ・ Mermaid diagrams ・ Prompt playbook')+'</p><p>© 2026 エンジニアリングのタネ制作委員会<br>by にしあん</p>'},
  ];
  const nav=$('helpNav');
