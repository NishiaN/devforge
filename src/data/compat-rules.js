...
const COMPAT_RULES=[
  ...
  {id:'be-dep-bun-netlify',p:['backend','deploy'],lv:'error',
   ...
   why_ja:'NetlifyのFunctionsはNode.jsとGoのみ公式サポートし、独立ランタイムのBunは動作しません。Bun固有API（Bun.serve・Bun.file等）が使えず、ビルドインフラもbunを認識しません。RailwayならBunのDockerコンテナを直接実行でき最も簡単な移行先です。',
  ...
  {id:'be-dep-express-netlify',p:['backend','deploy'],lv:'warn',
   ...
   why_ja:'NetlifyのFunctionsはリクエストごとに起動・終了するサーバーレス環境です。Expressは`netlify-lambda`でラップ可能ですが、WebSocket・セッション保持・FS書込などのステートフル機能が動作せず、実行時間上限（10秒）も多くの用途に不十分です。',
  ...
  {id:'obs-py-structlog',p:['backend'],lv:'info',
   ...
   why_ja:'Pythonの標準`logging`はJSON構造化ログに非対応です。`structlog`はプロセッサチェーンとJSONレンダラーを備え、`opentelemetry-sdk`と組合せOTelトレース連携（trace_id/span_id自動注入）が可能。`mask_sensitive()`でemail/token等PIIを`[REDACTED]`に置換すればGDPR対応も容易です。',
