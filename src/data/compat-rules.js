...
const COMPAT_RULES=[
  ...
  {id:'dom-manufacturing-noiot',p:['purpose'],lv:'info',
   ...
   why_ja:'工場の設備監視・スマートファクトリーシステムではPLC・センサー・OT機器とのデータ収集が中核機能です。MQTT（軽量M2Mプロトコル）やOPC-UA（産業用標準）を通じたIoTゲートウェイ連携を設計段階で検討することで、リアルタイム稼働監視・予知保全が実現できます。',
   why_en:'Equipment monitoring and smart factory systems require data collection from PLCs, sensors, and OT devices as a core function. Considering IoT gateway integration via MQTT (lightweight M2M protocol) or OPC-UA (industrial standard) at design stage enables real-time operational monitoring and predictive maintenance.'},
  {id:'dom-medical-noencrypt',p:['purpose','data_entities'],lv:'error',
   ...
   why_ja:'電子カルテ・EHRシステムは最高機密レベルの医療情報（診断・処方・検査結果）を扱います。HIPAA（米国）・個人情報保護法（日本）では保存データの暗号化（AES-256）と転送時のTLS 1.2+が要件です。DB全体暗号化（TDE）またはフィールドレベル暗号化を実装し、バックアップも同様に暗号化してください。',
   why_en:'EHR/clinical records systems handle the highest-confidentiality medical information (diagnoses, prescriptions, test results). HIPAA (US) and privacy laws (Japan) require encryption at rest (AES-256) and TLS 1.2+ in transit. Implement full-disk encryption (TDE) or field-level encryption, and encrypt backups equally.'},
  ...
  {id:'dom-education-nolti',p:['purpose'],lv:'info',
   ...
   why_ja:'LMSプラットフォームが教育機関向けである場合、Canvas・Moodle・Blackboard等の既存LMSとのシステム統合要求が高い可能性があります。LTI 1.3標準に対応することで外部ツールとのSSO・グレード同期・コンテンツ共有が容易になります。SCORM/xAPIも同様にコンテンツのポータビリティを保証します。',
   why_en:'When an LMS targets educational institutions, system integration requirements with existing LMS like Canvas, Moodle, and Blackboard are likely high. LTI 1.3 compliance enables SSO, grade sync, and content sharing with external tools. SCORM/xAPI similarly ensures content portability.'},
  {id:'dom-fintech-no2fa',p:['purpose'],lv:'error',
   ...
   why_ja:'金融庁ガイドライン・FISC安全対策基準・PCI DSS v4.0は銀行・証券システムのオンライン取引に対してMFA（多要素認証）を必須要件として規定しています。TOTP（FIDO2/WebAuthn）、ハードウェアトークン、SMS OTP（低リスク操作限定）の3レベルから適切なものを選択し、高額送金・ポートフォリオ変更等の重要操作は必ずMFAでゲートしてください。',
   why_en:'FSA guidelines, FISC security standards, and PCI DSS v4.0 mandate MFA for online transactions in banking and securities systems. Select appropriate MFA from three levels: TOTP (FIDO2/WebAuthn), hardware tokens, or SMS OTP (low-risk operations only). All critical operations like large transfers and portfolio changes must be MFA-gated.'},
  {id:'ai-medical-legal-noguard',p:['purpose','ai_auto'],lv:'warn',
   ...
   why_ja:'AI医療診断・法律判断はEU AI Act Annex III「高リスクAI」分類の対象となり、ヒトによる監視（HITL）・説明可能性・ハルシネーション検知が法的要件です。免責表示（「本システムは医師の判断を補助するものであり最終判断は医師が行う」等）を全出力に付与することが最低限必要です。',
   why_en:'AI medical diagnosis and legal decision systems fall under EU AI Act Annex III "High-Risk AI" classification, requiring human oversight (HITL), explainability, and hallucination detection. At minimum, add disclaimers to all outputs stating the system assists, not replaces, physician/legal judgment.'},
  {id:'fe-react-legacy-state',p:['frontend','mvp_features'],lv:'info',
   ...
   why_ja:'React 18+ではServerComponents・Suspense・use()フックにより状態管理の境界が変化しています。非Toolkit Reduxはアクション・リデューサー・セレクターの大量ボイラープレートが必要です。Zustandは3KB・ゼロボイラープレート、JotaiはアトミックなReactファースト状態管理、TanStack QueryはサーバーステートのキャッシュをSuspenseパターンで実装できます。',
   why_en:'React 18+ changes state management boundaries with ServerComponents, Suspense, and the use() hook. Non-Toolkit Redux requires large amounts of action/reducer/selector boilerplate. Zustand (3KB, zero boilerplate), Jotai (atomic React-first state), and TanStack Query (server state with Suspense) are modern alternatives.'},
  {id:'perf-large-entity-noindex',p:['data_entities'],lv:'warn',
   ...
   why_ja:'エンティティが12件以上になると外部キー結合・複合条件検索が増加し、インデックスなしではN+1クエリやフルスキャンが深刻なパフォーマンス問題となります。特に「ユーザーID+状態」「日付範囲+テナントID」のような複合インデックスは必須です。Prismaでは`@@index([field1, field2])`で複合インデックスを定義できます。',
   why_en:'12+ entities increase foreign key joins and multi-condition searches. Without indexes, N+1 queries and full scans become serious performance issues. Composite indexes on "user_id + status" and "date_range + tenant_id" patterns are critical. In Prisma, define composite indexes with `@@index([field1, field2])`.'},
  {id:'sec-sensitive-nobackup',p:['data_entities'],lv:'warn',
   ...
   why_en:'Medical/financial records like MedicalRecord have legal retention obligations (5-10 years), and legal liability for loss is extremely high. At minimum: configure Point-in-Time Recovery (PITR), daily snapshots, and geographically redundant backups, and document restoration procedures.'},
  {id:'cl-monorepo-notools',p:['dev_methods'],lv:'info',
   ...
   why_ja:'モノレポはリポジトリ管理を一元化できる反面、適切なツールなしではCI/CDが全パッケージをリビルドし開発速度が大幅に低下します。Turborepoはリモートキャッシュ・依存グラフに基づく差分ビルドで最大10倍の高速化が可能です。Nx（高機能）やpnpm workspace（シンプル・標準的）も有力な選択肢です。',
   why_en:'While monorepos centralize repository management, without proper tooling CI/CD rebuilds all packages, significantly slowing development. Turborepo enables up to 10x speedup via remote caching and incremental builds. Nx (feature-rich) and pnpm workspace (simple, standard) are also strong options.'},
  ...
  {id:'cl-api-noversioning',p:['backend'],lv:'info',
   ...
   why_ja:'大規模APIでクライアント（モバイルアプリ・外部連携）が増えると後方互換性のない変更のたびに全クライアントの一斉更新が必要になります。最初から `/api/v1/` のパスプレフィックスを設計に組み込むことで将来の段階移行が可能になります。FastAPIなら`APIRouter(prefix="/v1")`、NestJSなら`@Controller("v1/resource")`で実装できます。',
   why_en:'As large-scale APIs gain more clients, every breaking change requires simultaneous updates across all clients. Building `/api/v1/` path prefixes into the design enables gradual migration. Implement with `APIRouter(prefix="/v1")` in FastAPI or `@Controller("v1/resource")` in NestJS.'},
