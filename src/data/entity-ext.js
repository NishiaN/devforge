...
// ext21 standard preset entities
ENTITY_COLUMNS['GeotechSite']=['id:UUID:PK:調査地点ID:Geotech site ID','site_name:VARCHAR(200):NOT NULL:地点名:Site name','location:VARCHAR(300)::所在地:Location','latitude:DECIMAL(10,7)::緯度:Latitude','longitude:DECIMAL(10,7)::経度:Longitude','investigation_date:DATE::調査日:Investigation date','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['BoringLog']=['id:UUID:PK:ボーリングID:Boring log ID','site_id:UUID:FK:地点ID:Site ID','boring_no:VARCHAR(50):NOT NULL:ボーリング番号:Boring number','depth_m:DECIMAL(8,2)::掘削深度(m):Drilling depth (m)','groundwater_depth:DECIMAL(8,2)::地下水位(m):Groundwater depth (m)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SoilClassification']=['id:UUID:PK:土質分類ID:Soil classification ID','boring_id:UUID:FK:ボーリングID:Boring ID','depth_from:DECIMAL(8,2)::深度上端:Depth from','depth_to:DECIMAL(8,2)::深度下端:Depth to','soil_type:VARCHAR(100):NOT NULL:土質種別:Soil type','n_value:INTEGER::N値:N value','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GroundwaterTable']=['id:UUID:PK:地下水位記録ID:Groundwater table ID','site_id:UUID:FK:地点ID:Site ID','measured_at:DATE:NOT NULL:計測日:Measured at','depth_m:DECIMAL(8,3):NOT NULL:水位深度(m):Water table depth (m)','measurement_method:VARCHAR(100)::計測方法:Measurement method','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['LiquefactionZone']=['id:UUID:PK:液状化ゾーンID:Liquefaction zone ID','site_id:UUID:FK:地点ID:Site ID','risk_level:VARCHAR(50):NOT NULL:リスクレベル:Risk level','fl_value:DECIMAL(6,3)::FL値:FL value','depth_range:VARCHAR(50)::対象深度範囲:Depth range','mitigation_method:VARCHAR(200)::対策工法:Mitigation method','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['HealthCheckup']=['id:UUID:PK:健康診断ID:Health checkup ID','examinee_id:UUID:FK:受診者ID:Examinee ID','checkup_date:DATE:NOT NULL:受診日:Checkup date','checkup_type:VARCHAR(100):NOT NULL:健診種別:Checkup type','institution:VARCHAR(200)::実施機関:Institution','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CheckupResult']=['id:UUID:PK:検査結果ID:Checkup result ID','checkup_id:UUID:FK:健診ID:Checkup ID','item_name:VARCHAR(200):NOT NULL:検査項目名:Item name','value:VARCHAR(100)::測定値:Value','unit:VARCHAR(50)::単位:Unit','reference_range:VARCHAR(100)::基準範囲:Reference range','judgment:VARCHAR(20)::判定:Judgment','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['HbA1cTrend']=['id:UUID:PK:HbA1cトレンドID:HbA1c trend ID','examinee_id:UUID:FK:受診者ID:Examinee ID','measured_date:DATE:NOT NULL:計測日:Measured date','hba1c_value:DECIMAL(4,1):NOT NULL:HbA1c値(%):HbA1c value (%)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['RiskCategory']=['id:UUID:PK:リスクカテゴリID:Risk category ID','examinee_id:UUID:FK:受診者ID:Examinee ID','disease_type:VARCHAR(100):NOT NULL:疾患種別:Disease type','risk_score:DECIMAL(5,2):NOT NULL:リスクスコア:Risk score','risk_level:VARCHAR(20):NOT NULL:リスクレベル:Risk level','evaluated_at:DATE:NOT NULL:評価日:Evaluated at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['HealthCoachPlan']=['id:UUID:PK:保健指導計画ID:Health coach plan ID','examinee_id:UUID:FK:受診者ID:Examinee ID','plan_content:TEXT::指導内容:Plan content','goal:VARCHAR(500)::目標:Goal','target_date:DATE::目標達成日:Target date','status:VARCHAR(50):DEFAULT \'active\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PatientConstitution']=['id:UUID:PK:患者体質ID:Patient constitution ID','patient_id:UUID:FK:患者ID:Patient ID','sho_type:VARCHAR(100):NOT NULL:証タイプ:Sho type','constitution_score:INTEGER::体質スコア:Constitution score','main_symptoms:TEXT::主訴:Main symptoms','tongue_findings:TEXT::舌診所見:Tongue findings','pulse_findings:TEXT::脈診所見:Pulse findings','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['KampoPrescrip']=['id:UUID:PK:漢方処方ID:Kampo prescription ID','patient_id:UUID:FK:患者ID:Patient ID','formula_name:VARCHAR(200):NOT NULL:方剤名:Formula name','prescribed_at:DATE:NOT NULL:処方日:Prescribed date','duration_days:INTEGER::処方日数:Duration days','dosage_notes:TEXT::服用方法:Dosage notes','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ShoHerb']=['id:UUID:PK:生薬ID:Herb ID','herb_name_ja:VARCHAR(200):NOT NULL:生薬名:Herb name (JA)','herb_name_en:VARCHAR(200)::英名:Herb name (EN)','category:VARCHAR(100)::分類:Category','actions:TEXT::薬効:Actions','contraindications:TEXT::禁忌:Contraindications','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TreatmentRecord']=['id:UUID:PK:治療記録ID:Treatment record ID','patient_id:UUID:FK:患者ID:Patient ID','treatment_date:DATE:NOT NULL:治療日:Treatment date','treatment_type:VARCHAR(200):NOT NULL:治療種別:Treatment type','effect_rating:INTEGER::効果評価(1-5):Effect rating (1-5)','notes:TEXT::記録内容:Notes','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ConsultNote']=['id:UUID:PK:診察メモID:Consult note ID','patient_id:UUID:FK:患者ID:Patient ID','consulted_at:TIMESTAMP:NOT NULL:診察日時:Consulted at','chief_complaint:TEXT::主訴:Chief complaint','assessment:TEXT::評価:Assessment','plan:TEXT::治療計画:Plan','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SchoolGuardian']=['id:UUID:PK:保護者ID:School guardian ID','student_name:VARCHAR(200):NOT NULL:児童生徒名:Student name','grade_class:VARCHAR(50):NOT NULL:学年・組:Grade/class','guardian_name:VARCHAR(200):NOT NULL:保護者名:Guardian name','contact_email:VARCHAR(300)::連絡先メール:Contact email','contact_phone:VARCHAR(30)::電話番号:Phone number','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ClassReport']=['id:UUID:PK:学級通信ID:Class report ID','class_id:VARCHAR(50):NOT NULL:クラスID:Class ID','title:VARCHAR(300):NOT NULL:タイトル:Title','content:TEXT:NOT NULL:内容:Content','published_at:TIMESTAMP::発行日時:Published at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['AttendanceRecord']=['id:UUID:PK:出欠記録ID:Attendance record ID','student_id:UUID:FK:生徒ID:Student ID','record_date:DATE:NOT NULL:記録日:Record date','status:VARCHAR(30):NOT NULL:出欠区分:Status','arrival_time:TIME::登校時刻:Arrival time','reason:VARCHAR(200)::事由:Reason','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GuardianMessage']=['id:UUID:PK:保護者メッセージID:Guardian message ID','guardian_id:UUID:FK:保護者ID:Guardian ID','message_type:VARCHAR(100):NOT NULL:メッセージ種別:Message type','subject:VARCHAR(300):NOT NULL:件名:Subject','body:TEXT:NOT NULL:本文:Body','sent_at:TIMESTAMP::送信日時:Sent at','read_at:TIMESTAMP::既読日時:Read at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TeacherTask']=['id:UUID:PK:教員タスクID:Teacher task ID','teacher_id:UUID:FK:教員ID:Teacher ID','task_title:VARCHAR(300):NOT NULL:タスク名:Task title','due_date:DATE::期日:Due date','category:VARCHAR(100)::カテゴリ:Category','status:VARCHAR(50):DEFAULT \'pending\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['VideoEditProject']=['id:UUID:PK:動画プロジェクトID:Video edit project ID','owner_id:UUID:FK:オーナーID:Owner ID','title:VARCHAR(300):NOT NULL:プロジェクト名:Project title','source_url:TEXT::元動画URL:Source video URL','duration_sec:INTEGER::動画長(秒):Duration (sec)','status:VARCHAR(50):DEFAULT \'draft\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ClipSegment']=['id:UUID:PK:クリップID:Clip segment ID','project_id:UUID:FK:プロジェクトID:Project ID','start_sec:DECIMAL(10,3):NOT NULL:開始秒:Start sec','end_sec:DECIMAL(10,3):NOT NULL:終了秒:End sec','label:VARCHAR(200)::ラベル:Label','highlight_score:DECIMAL(4,2)::ハイライトスコア:Highlight score','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CaptionTrack']=['id:UUID:PK:テロップID:Caption track ID','project_id:UUID:FK:プロジェクトID:Project ID','language:VARCHAR(20):NOT NULL:言語コード:Language code','content_json:TEXT::字幕JSON:Caption JSON','generated_at:TIMESTAMP::生成日時:Generated at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ShortVideoExport']=['id:UUID:PK:ショート動画ID:Short video export ID','project_id:UUID:FK:プロジェクトID:Project ID','platform:VARCHAR(50):NOT NULL:プラットフォーム:Platform','aspect_ratio:VARCHAR(20):NOT NULL:アスペクト比:Aspect ratio','export_url:TEXT::出力URL:Export URL','scheduled_at:TIMESTAMP::投稿予定日時:Scheduled at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ThumbnailAsset']=['id:UUID:PK:サムネイルID:Thumbnail asset ID','project_id:UUID:FK:プロジェクトID:Project ID','image_url:TEXT:NOT NULL:画像URL:Image URL','variant:VARCHAR(50)::バリアント:Variant','selected:BOOLEAN:DEFAULT FALSE:選択済:Selected','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['MonitorStation']=['id:UUID:PK:監視ステーションID:Monitor station ID','name:VARCHAR(200):NOT NULL:ステーション名:Station name','location:VARCHAR(300)::所在地:Location','station_type:VARCHAR(100):NOT NULL:ステーション種別:Station type','latitude:DECIMAL(10,7)::緯度:Latitude','longitude:DECIMAL(10,7)::経度:Longitude','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['WaterSensor']=['id:UUID:PK:水質センサーID:Water sensor ID','station_id:UUID:FK:ステーションID:Station ID','sensor_type:VARCHAR(100):NOT NULL:センサー種別:Sensor type','parameter_name:VARCHAR(100):NOT NULL:パラメータ名:Parameter name','unit:VARCHAR(30)::単位:Unit','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['QualityMeasurement']=['id:UUID:PK:品質計測ID:Quality measurement ID','sensor_id:UUID:FK:センサーID:Sensor ID','measured_at:TIMESTAMP:NOT NULL:計測日時:Measured at','value:DECIMAL(12,4):NOT NULL:計測値:Measured value','exceeds_threshold:BOOLEAN:DEFAULT FALSE:閾値超過:Exceeds threshold','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['AlertEvent']=['id:UUID:PK:アラートイベントID:Alert event ID','station_id:UUID:FK:ステーションID:Station ID','alert_type:VARCHAR(100):NOT NULL:アラート種別:Alert type','severity:VARCHAR(50):NOT NULL:重大度:Severity','triggered_at:TIMESTAMP:NOT NULL:発生日時:Triggered at','resolved_at:TIMESTAMP::解決日時:Resolved at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['AirSensor']=['id:UUID:PK:大気センサーID:Air sensor ID','station_id:UUID:FK:ステーションID:Station ID','pollutant:VARCHAR(100):NOT NULL:汚染物質:Pollutant','unit:VARCHAR(30)::単位:Unit','threshold_warn:DECIMAL(10,3)::警告閾値:Warning threshold','threshold_crit:DECIMAL(10,3)::危険閾値:Critical threshold','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ChildDev']=['id:UUID:PK:子どもID:Child dev ID','family_id:UUID:FK:家族ID:Family ID','birth_date:DATE:NOT NULL:誕生日:Birth date','gender:VARCHAR(20)::性別:Gender','full_name:VARCHAR(200):NOT NULL:氏名:Full name','notes:TEXT::特記事項:Notes','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['DevelopmentMilestone']=['id:UUID:PK:発達マイルストーンID:Development milestone ID','child_id:UUID:FK:子どもID:Child ID','milestone_name:VARCHAR(200):NOT NULL:マイルストーン名:Milestone name','age_months_typical:INTEGER::標準達成月齢:Typical age months','achieved_at:DATE::達成日:Achieved date','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GrowthRecord']=['id:UUID:PK:成長記録ID:Growth record ID','child_id:UUID:FK:子どもID:Child ID','recorded_at:DATE:NOT NULL:記録日:Recorded at','height_cm:DECIMAL(5,1)::身長(cm):Height (cm)','weight_kg:DECIMAL(5,2)::体重(kg):Weight (kg)','head_cm:DECIMAL(5,1)::頭囲(cm):Head circumference (cm)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['NurseryVisit']=['id:UUID:PK:保育訪問ID:Nursery visit ID','child_id:UUID:FK:子どもID:Child ID','visit_date:DATE:NOT NULL:訪問日:Visit date','visitor_role:VARCHAR(100)::訪問者種別:Visitor role','observations:TEXT::観察記録:Observations','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ParentNote']=['id:UUID:PK:保護者記録ID:Parent note ID','child_id:UUID:FK:子どもID:Child ID','noted_at:DATE:NOT NULL:記録日:Noted at','category:VARCHAR(100)::カテゴリ:Category','content:TEXT:NOT NULL:内容:Content','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ExperienceProgram']=['id:UUID:PK:体験プログラムID:Experience program ID','host_id:UUID:FK:提供者ID:Host ID','title:VARCHAR(300):NOT NULL:タイトル:Title','category:VARCHAR(100):NOT NULL:カテゴリ:Category','duration_min:INTEGER::所要時間(分):Duration (min)','capacity:INTEGER::定員:Capacity','price_jpy:INTEGER:NOT NULL:価格(円):Price (JPY)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['Workshop']=['id:UUID:PK:ワークショップID:Workshop ID','program_id:UUID:FK:プログラムID:Program ID','scheduled_at:TIMESTAMP:NOT NULL:開催日時:Scheduled at','venue:VARCHAR(300)::開催場所:Venue','status:VARCHAR(50):DEFAULT \'scheduled\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['HostProvider']=['id:UUID:PK:提供者ID:Host provider ID','name:VARCHAR(200):NOT NULL:提供者名:Host name','specialty:VARCHAR(200)::専門分野:Specialty','description:TEXT::紹介:Description','verified:BOOLEAN:DEFAULT FALSE:認証済:Verified','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['BookingSlot']=['id:UUID:PK:予約スロットID:Booking slot ID','workshop_id:UUID:FK:ワークショップID:Workshop ID','participant_id:UUID:FK:参加者ID:Participant ID','booked_at:TIMESTAMP:NOT NULL:予約日時:Booked at','status:VARCHAR(50):DEFAULT \'confirmed\':ステータス:Status','payment_status:VARCHAR(50)::決済ステータス:Payment status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ReviewEntry']=['id:UUID:PK:レビューID:Review entry ID','workshop_id:UUID:FK:ワークショップID:Workshop ID','participant_id:UUID:FK:参加者ID:Participant ID','rating:INTEGER:NOT NULL:評価(1-5):Rating (1-5)','comment:TEXT::コメント:Comment','submitted_at:TIMESTAMP:NOT NULL:投稿日時:Submitted at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['EnergyMeter']=['id:UUID:PK:電力計ID:Energy meter ID','factory_id:UUID:FK:工場ID:Factory ID','meter_name:VARCHAR(200):NOT NULL:計器名:Meter name','location:VARCHAR(200)::設置場所:Location','meter_type:VARCHAR(100):NOT NULL:計器種別:Meter type','capacity_kw:DECIMAL(10,2)::定格容量(kW):Rated capacity (kW)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ConsumptionLog']=['id:UUID:PK:消費ログID:Consumption log ID','meter_id:UUID:FK:計器ID:Meter ID','logged_at:TIMESTAMP:NOT NULL:記録日時:Logged at','consumption_kwh:DECIMAL(12,3):NOT NULL:消費量(kWh):Consumption (kWh)','demand_kw:DECIMAL(10,2)::デマンド(kW):Demand (kW)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PeakDemandEvent']=['id:UUID:PK:ピークデマンドID:Peak demand event ID','factory_id:UUID:FK:工場ID:Factory ID','peak_at:TIMESTAMP:NOT NULL:ピーク日時:Peak time','peak_kw:DECIMAL(10,2):NOT NULL:ピーク電力(kW):Peak demand (kW)','contract_kw:DECIMAL(10,2)::契約電力(kW):Contract demand (kW)','exceeded:BOOLEAN:DEFAULT FALSE:超過:Exceeded','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['EnergyUsageReport']=['id:UUID:PK:エネルギー報告ID:Energy usage report ID','factory_id:UUID:FK:工場ID:Factory ID','period_month:VARCHAR(10):NOT NULL:対象月:Period month','total_kwh:DECIMAL(14,2)::合計電力量(kWh):Total kWh','total_co2_kg:DECIMAL(12,2)::CO2排出量(kg):Total CO2 (kg)','savings_kwh:DECIMAL(12,2)::削減量(kWh):Savings (kWh)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SavingTarget']=['id:UUID:PK:省エネ目標ID:Saving target ID','factory_id:UUID:FK:工場ID:Factory ID','fiscal_year:INTEGER:NOT NULL:年度:Fiscal year','target_reduction_pct:DECIMAL(5,2):NOT NULL:削減目標(%):Target reduction (%)','baseline_kwh:DECIMAL(14,2)::基準電力量(kWh):Baseline (kWh)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SkillHolder']=['id:UUID:PK:技能保有者ID:Skill holder ID','name:VARCHAR(200):NOT NULL:氏名:Name','department:VARCHAR(200)::部署:Department','years_experience:INTEGER::経験年数:Years of experience','expertise_area:VARCHAR(300)::専門領域:Expertise area','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['KnowledgeArticle']=['id:UUID:PK:技能記事ID:Knowledge article ID','holder_id:UUID:FK:技能者ID:Holder ID','title:VARCHAR(300):NOT NULL:タイトル:Title','content:TEXT:NOT NULL:内容:Content','difficulty_level:INTEGER::難易度:Difficulty level','tags:TEXT::タグ:Tags','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SkillWorkProcess']=['id:UUID:PK:作業工程ID:Skill work process ID','article_id:UUID:FK:記事ID:Article ID','step_no:INTEGER:NOT NULL:ステップ番号:Step number','step_title:VARCHAR(300):NOT NULL:手順名:Step title','description:TEXT::説明:Description','video_url:TEXT::動画URL:Video URL','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['LearnerEvaluation']=['id:UUID:PK:学習評価ID:Learner evaluation ID','learner_id:UUID:FK:学習者ID:Learner ID','article_id:UUID:FK:記事ID:Article ID','score:INTEGER::スコア:Score','evaluated_at:DATE:NOT NULL:評価日:Evaluated at','notes:TEXT::コメント:Notes','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SkillRecord']=['id:UUID:PK:技能記録ID:Skill record ID','learner_id:UUID:FK:学習者ID:Learner ID','skill_name:VARCHAR(200):NOT NULL:技能名:Skill name','level:VARCHAR(50):NOT NULL:習熟レベル:Proficiency level','certified_at:DATE::認定日:Certified at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
// ext21 field preset entities
ENTITY_COLUMNS['GeotechDataset']=['id:UUID:PK:地質データセットID:Geotech dataset ID','project_id:UUID:FK:プロジェクトID:Project ID','dataset_name:VARCHAR(200):NOT NULL:データセット名:Dataset name','data_type:VARCHAR(100):NOT NULL:データ種別:Data type','source:VARCHAR(200)::データ提供元:Source','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['BoreholeSample']=['id:UUID:PK:ボーリングサンプルID:Borehole sample ID','boring_id:UUID:FK:ボーリングID:Boring ID','sample_depth_m:DECIMAL(8,2):NOT NULL:採取深度(m):Sample depth (m)','sample_type:VARCHAR(100):NOT NULL:試料種別:Sample type','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SoilTestBatch']=['id:UUID:PK:土質試験バッチID:Soil test batch ID','sample_id:UUID:FK:サンプルID:Sample ID','test_type:VARCHAR(100):NOT NULL:試験種別:Test type','test_date:DATE:NOT NULL:試験日:Test date','lab_name:VARCHAR(200)::試験機関:Lab name','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GeophysicalSurvey']=['id:UUID:PK:物理探査ID:Geophysical survey ID','site_id:UUID:FK:地点ID:Site ID','method:VARCHAR(100):NOT NULL:探査手法:Survey method','survey_date:DATE:NOT NULL:実施日:Survey date','data_url:TEXT::データURL:Data URL','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GroundModel']=['id:UUID:PK:地盤モデルID:Ground model ID','site_id:UUID:FK:地点ID:Site ID','model_type:VARCHAR(100):NOT NULL:モデル種別:Model type','layers_json:TEXT::地層JSON:Layers JSON','generated_at:TIMESTAMP:NOT NULL:生成日時:Generated at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CheckupAnalysis']=['id:UUID:PK:健診解析ID:Checkup analysis ID','checkup_id:UUID:FK:健診ID:Checkup ID','model_version:VARCHAR(50)::モデルバージョン:Model version','risk_factors:TEXT::リスク因子JSON:Risk factors JSON','analyzed_at:TIMESTAMP:NOT NULL:解析日時:Analyzed at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['HealthTrend']=['id:UUID:PK:健康トレンドID:Health trend ID','examinee_id:UUID:FK:受診者ID:Examinee ID','item_name:VARCHAR(200):NOT NULL:指標名:Item name','trend_direction:VARCHAR(20)::トレンド方向:Trend direction','change_rate:DECIMAL(8,4)::変化率:Change rate','period_months:INTEGER::対象期間(月):Period months','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['RiskScore']=['id:UUID:PK:リスクスコアID:Risk score ID','examinee_id:UUID:FK:受診者ID:Examinee ID','disease_category:VARCHAR(100):NOT NULL:疾患カテゴリ:Disease category','score:DECIMAL(6,3):NOT NULL:スコア:Score','percentile:DECIMAL(5,2)::パーセンタイル:Percentile','scored_at:DATE:NOT NULL:算出日:Scored at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['InterventionPlan']=['id:UUID:PK:介入計画ID:Intervention plan ID','examinee_id:UUID:FK:受診者ID:Examinee ID','intervention_type:VARCHAR(100):NOT NULL:介入種別:Intervention type','start_date:DATE:NOT NULL:開始日:Start date','end_date:DATE::終了日:End date','goals:TEXT::目標:Goals','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PopHealthCohort']=['id:UUID:PK:コホートID:Pop health cohort ID','cohort_name:VARCHAR(200):NOT NULL:コホート名:Cohort name','criteria_json:TEXT::選定条件JSON:Criteria JSON','size:INTEGER::コホートサイズ:Cohort size','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ConstitutionType']=['id:UUID:PK:体質タイプID:Constitution type ID','type_name:VARCHAR(100):NOT NULL:体質名:Type name','type_code:VARCHAR(50):NOT NULL:タイプコード:Type code','characteristics:TEXT::特徴:Characteristics','suitable_formulas:TEXT::適合方剤:Suitable formulas','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['HerbFormula']=['id:UUID:PK:方剤ID:Herb formula ID','formula_name:VARCHAR(200):NOT NULL:方剤名:Formula name','formula_code:VARCHAR(100)::方剤コード:Formula code','constituents:TEXT::構成生薬:Constituents','indications:TEXT::適応証:Indications','evidence_level:VARCHAR(50)::エビデンスレベル:Evidence level','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ConsultSession']=['id:UUID:PK:診察セッションID:Consult session ID','patient_id:UUID:FK:患者ID:Patient ID','practitioner_id:UUID:FK:施術者ID:Practitioner ID','session_date:DATE:NOT NULL:診察日:Session date','duration_min:INTEGER::診察時間(分):Duration (min)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TherapyOutcome']=['id:UUID:PK:治療転帰ID:Therapy outcome ID','session_id:UUID:FK:セッションID:Session ID','outcome_type:VARCHAR(100):NOT NULL:転帰種別:Outcome type','symptom_change:VARCHAR(100)::症状変化:Symptom change','patient_satisfaction:INTEGER::患者満足度:Patient satisfaction','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['EvidenceRef']=['id:UUID:PK:エビデンス参照ID:Evidence ref ID','formula_id:UUID:FK:方剤ID:Formula ID','publication_title:VARCHAR(500):NOT NULL:論文タイトル:Publication title','doi:VARCHAR(200)::DOI:DOI','evidence_grade:VARCHAR(50)::グレード:Evidence grade','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SchoolEvent']=['id:UUID:PK:学校行事ID:School event ID','event_name:VARCHAR(300):NOT NULL:行事名:Event name','event_date:DATE:NOT NULL:開催日:Event date','target_grade:VARCHAR(50)::対象学年:Target grade','description:TEXT::概要:Description','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['NotificationRecord']=['id:UUID:PK:通知記録ID:Notification record ID','guardian_id:UUID:FK:保護者ID:Guardian ID','subject:VARCHAR(300):NOT NULL:件名:Subject','sent_at:TIMESTAMP:NOT NULL:送信日時:Sent at','read_at:TIMESTAMP::既読日時:Read at','channel:VARCHAR(50)::チャネル:Channel','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GuardianConsent']=['id:UUID:PK:保護者同意ID:Guardian consent ID','guardian_id:UUID:FK:保護者ID:Guardian ID','consent_type:VARCHAR(200):NOT NULL:同意種別:Consent type','consented_at:TIMESTAMP::同意日時:Consented at','status:VARCHAR(50):DEFAULT \'pending\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['StaffTask']=['id:UUID:PK:スタッフタスクID:Staff task ID','assignee_id:UUID:FK:担当者ID:Assignee ID','title:VARCHAR(300):NOT NULL:タスク名:Task title','due_date:DATE::期日:Due date','priority:VARCHAR(20):DEFAULT \'normal\':優先度:Priority','completed_at:TIMESTAMP::完了日時:Completed at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['StudentRecord']=['id:UUID:PK:生徒記録ID:Student record ID','student_name:VARCHAR(200):NOT NULL:氏名:Student name','grade:VARCHAR(20):NOT NULL:学年:Grade','class_code:VARCHAR(20):NOT NULL:組:Class','enrollment_date:DATE:NOT NULL:入学日:Enrollment date','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['VideoSource']=['id:UUID:PK:映像素材ID:Video source ID','owner_id:UUID:FK:オーナーID:Owner ID','file_url:TEXT:NOT NULL:ファイルURL:File URL','duration_sec:INTEGER::動画長(秒):Duration sec','resolution:VARCHAR(50)::解像度:Resolution','uploaded_at:TIMESTAMP:NOT NULL:アップロード日時:Uploaded at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['EditSession']=['id:UUID:PK:編集セッションID:Edit session ID','source_id:UUID:FK:素材ID:Source ID','editor_id:UUID:FK:編集者ID:Editor ID','started_at:TIMESTAMP:NOT NULL:開始日時:Started at','completed_at:TIMESTAMP::完了日時:Completed at','status:VARCHAR(50):DEFAULT \'in_progress\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ExportFormat']=['id:UUID:PK:エクスポート形式ID:Export format ID','session_id:UUID:FK:セッションID:Session ID','format_type:VARCHAR(50):NOT NULL:形式:Format type','resolution:VARCHAR(50)::解像度:Resolution','bitrate_kbps:INTEGER::ビットレート(kbps):Bitrate (kbps)','output_url:TEXT::出力URL:Output URL','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['DistributionChannel']=['id:UUID:PK:配信チャネルID:Distribution channel ID','platform:VARCHAR(100):NOT NULL:プラットフォーム:Platform','account_name:VARCHAR(200)::アカウント名:Account name','connected:BOOLEAN:DEFAULT FALSE:連携済:Connected','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ContentPerformance']=['id:UUID:PK:パフォーマンスID:Content performance ID','export_id:UUID:FK:エクスポートID:Export ID','views:BIGINT:DEFAULT 0:再生数:Views','likes:INTEGER:DEFAULT 0:いいね数:Likes','shares:INTEGER:DEFAULT 0:シェア数:Shares','recorded_at:TIMESTAMP:NOT NULL:記録日時:Recorded at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SensorNetwork']=['id:UUID:PK:センサーネットワークID:Sensor network ID','network_name:VARCHAR(200):NOT NULL:ネットワーク名:Network name','region:VARCHAR(200)::対象地域:Region','sensor_count:INTEGER:DEFAULT 0:センサー数:Sensor count','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['WaterQualityParam']=['id:UUID:PK:水質パラメータID:Water quality param ID','network_id:UUID:FK:ネットワークID:Network ID','param_name:VARCHAR(100):NOT NULL:パラメータ名:Parameter name','unit:VARCHAR(30)::単位:Unit','standard_limit:DECIMAL(12,4)::基準値:Standard limit','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['AirQualityParam']=['id:UUID:PK:大気パラメータID:Air quality param ID','network_id:UUID:FK:ネットワークID:Network ID','pollutant:VARCHAR(100):NOT NULL:汚染物質:Pollutant','unit:VARCHAR(30)::単位:Unit','standard_limit:DECIMAL(12,4)::環境基準値:Standard limit','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['AlertRule']=['id:UUID:PK:アラートルールID:Alert rule ID','network_id:UUID:FK:ネットワークID:Network ID','rule_name:VARCHAR(200):NOT NULL:ルール名:Rule name','condition_json:TEXT::条件JSON:Condition JSON','severity:VARCHAR(50):NOT NULL:重大度:Severity','enabled:BOOLEAN:DEFAULT TRUE:有効:Enabled','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['EcoImpactRecord']=['id:UUID:PK:生態影響記録ID:Eco impact record ID','network_id:UUID:FK:ネットワークID:Network ID','species_affected:VARCHAR(300)::影響生物種:Species affected','impact_type:VARCHAR(100):NOT NULL:影響種別:Impact type','severity_score:DECIMAL(4,2)::影響度スコア:Severity score','recorded_at:DATE:NOT NULL:記録日:Recorded at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ChildProfile']=['id:UUID:PK:子どもプロファイルID:Child profile ID','birth_date:DATE:NOT NULL:誕生日:Birth date','gender:VARCHAR(20)::性別:Gender','primary_caregiver:VARCHAR(200)::主養育者:Primary caregiver','support_level:VARCHAR(50)::支援レベル:Support level','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['MilestoneAssessment']=['id:UUID:PK:マイルストーン評価ID:Milestone assessment ID','child_id:UUID:FK:子どもID:Child ID','domain:VARCHAR(100):NOT NULL:発達領域:Domain','assessed_at:DATE:NOT NULL:評価日:Assessed at','achievement_level:VARCHAR(50)::達成レベル:Achievement level','assessor_role:VARCHAR(100)::評価者種別:Assessor role','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GrowthData']=['id:UUID:PK:成長データID:Growth data ID','child_id:UUID:FK:子どもID:Child ID','measured_at:DATE:NOT NULL:計測日:Measured at','height_cm:DECIMAL(5,1)::身長(cm):Height (cm)','weight_kg:DECIMAL(5,2)::体重(kg):Weight (kg)','percentile_height:DECIMAL(5,2)::身長パーセンタイル:Height percentile','percentile_weight:DECIMAL(5,2)::体重パーセンタイル:Weight percentile','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SupportNeeds']=['id:UUID:PK:支援ニーズID:Support needs ID','child_id:UUID:FK:子どもID:Child ID','need_category:VARCHAR(100):NOT NULL:ニーズカテゴリ:Need category','description:TEXT::詳細:Description','priority:VARCHAR(20):DEFAULT \'normal\':優先度:Priority','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['MultidiscTeam']=['id:UUID:PK:多職種チームID:Multidisc team ID','child_id:UUID:FK:子どもID:Child ID','member_name:VARCHAR(200):NOT NULL:氏名:Member name','role:VARCHAR(100):NOT NULL:役職:Role','organization:VARCHAR(200)::所属機関:Organization','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CulturalProgram']=['id:UUID:PK:文化プログラムID:Cultural program ID','title:VARCHAR(300):NOT NULL:タイトル:Title','category:VARCHAR(100):NOT NULL:カテゴリ:Category','region:VARCHAR(200)::地域:Region','description:TEXT::説明:Description','price_jpy:INTEGER:NOT NULL:価格(円):Price (JPY)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ExperienceHost']=['id:UUID:PK:体験ホストID:Experience host ID','name:VARCHAR(200):NOT NULL:ホスト名:Host name','bio:TEXT::自己紹介:Bio','rating:DECIMAL(3,2)::評価:Rating','verified:BOOLEAN:DEFAULT FALSE:認証済:Verified','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TouristBooking']=['id:UUID:PK:観光予約ID:Tourist booking ID','program_id:UUID:FK:プログラムID:Program ID','tourist_id:UUID:FK:旅行者ID:Tourist ID','booked_at:TIMESTAMP:NOT NULL:予約日時:Booked at','participants:INTEGER:NOT NULL:参加人数:Participants','total_price_jpy:INTEGER:NOT NULL:合計金額(円):Total price (JPY)','status:VARCHAR(50):DEFAULT \'confirmed\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ActivityReview']=['id:UUID:PK:活動レビューID:Activity review ID','program_id:UUID:FK:プログラムID:Program ID','reviewer_id:UUID:FK:レビュアーID:Reviewer ID','rating:INTEGER:NOT NULL:評価(1-5):Rating (1-5)','title:VARCHAR(300)::タイトル:Title','body:TEXT::レビュー本文:Body','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['LocalGuide']=['id:UUID:PK:ローカルガイドID:Local guide ID','name:VARCHAR(200):NOT NULL:氏名:Name','specialty:VARCHAR(200)::専門分野:Specialty','languages:VARCHAR(300)::対応言語:Languages','certified:BOOLEAN:DEFAULT FALSE:認定済:Certified','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['FactoryZone']=['id:UUID:PK:工場ゾーンID:Factory zone ID','factory_id:UUID:FK:工場ID:Factory ID','zone_name:VARCHAR(200):NOT NULL:ゾーン名:Zone name','zone_type:VARCHAR(100):NOT NULL:ゾーン種別:Zone type','area_sqm:DECIMAL(10,2)::面積(㎡):Area (sqm)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['EnergyDevice']=['id:UUID:PK:エネルギー機器ID:Energy device ID','zone_id:UUID:FK:ゾーンID:Zone ID','device_name:VARCHAR(200):NOT NULL:機器名:Device name','device_type:VARCHAR(100):NOT NULL:機器種別:Device type','rated_power_kw:DECIMAL(10,2)::定格電力(kW):Rated power (kW)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['LoadProfile']=['id:UUID:PK:負荷プロファイルID:Load profile ID','device_id:UUID:FK:機器ID:Device ID','period:VARCHAR(50):NOT NULL:期間:Period','avg_load_kw:DECIMAL(10,2)::平均負荷(kW):Average load (kW)','peak_load_kw:DECIMAL(10,2)::ピーク負荷(kW):Peak load (kW)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['OptimizationPlan']=['id:UUID:PK:最適化計画ID:Optimization plan ID','factory_id:UUID:FK:工場ID:Factory ID','plan_name:VARCHAR(300):NOT NULL:計画名:Plan name','expected_savings_kwh:DECIMAL(12,2)::期待削減量(kWh):Expected savings (kWh)','implementation_cost_jpy:INTEGER::実施コスト(円):Implementation cost (JPY)','status:VARCHAR(50):DEFAULT \'proposed\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CarbonRecord']=['id:UUID:PK:カーボン記録ID:Carbon record ID','factory_id:UUID:FK:工場ID:Factory ID','period_month:VARCHAR(10):NOT NULL:対象月:Period month','scope1_kg:DECIMAL(12,2)::Scope1排出(kg):Scope1 (kg)','scope2_kg:DECIMAL(12,2)::Scope2排出(kg):Scope2 (kg)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SkilledWorker']=['id:UUID:PK:熟練工ID:Skilled worker ID','name:VARCHAR(200):NOT NULL:氏名:Name','department:VARCHAR(200)::部署:Department','years_experience:INTEGER::経験年数:Years of experience','expertise_tags:TEXT::専門タグ:Expertise tags','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TechniqueCapture']=['id:UUID:PK:技能キャプチャID:Technique capture ID','worker_id:UUID:FK:熟練工ID:Worker ID','video_url:TEXT:NOT NULL:動画URL:Video URL','recorded_at:TIMESTAMP:NOT NULL:収録日時:Recorded at','skill_category:VARCHAR(200):NOT NULL:技能カテゴリ:Skill category','duration_sec:INTEGER::収録時間(秒):Duration sec','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TrainingModule']=['id:UUID:PK:訓練モジュールID:Training module ID','capture_id:UUID:FK:キャプチャID:Capture ID','title:VARCHAR(300):NOT NULL:タイトル:Title','difficulty_level:INTEGER:NOT NULL:難易度:Difficulty level','estimated_hours:DECIMAL(5,1)::推定学習時間(h):Estimated hours','content_json:TEXT::コンテンツJSON:Content JSON','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CompetencyMap']=['id:UUID:PK:コンピテンシーマップID:Competency map ID','department:VARCHAR(200):NOT NULL:部署:Department','skill_name:VARCHAR(200):NOT NULL:スキル名:Skill name','required_level:INTEGER:NOT NULL:必要習熟度:Required level','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TraineeFeedback']=['id:UUID:PK:受講者フィードバックID:Trainee feedback ID','module_id:UUID:FK:モジュールID:Module ID','trainee_id:UUID:FK:受講者ID:Trainee ID','rating:INTEGER:NOT NULL:評価(1-5):Rating (1-5)','comment:TEXT::コメント:Comment','submitted_at:TIMESTAMP:NOT NULL:投稿日時:Submitted at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

// ── ext22: presets-ext22.js new entity columns ────────────────────────────────

// Standard preset entities
ENTITY_COLUMNS['GapFarmField']=['id:UUID:PK:圃場ID:Farm field ID','owner_id:UUID:FK:オーナーID:Owner ID','field_name:VARCHAR(200):NOT NULL:圃場名:Field name','area_ha:DECIMAL(8,3)::面積(ha):Area (ha)','crop_type:VARCHAR(100)::作物種別:Crop type','location:VARCHAR(300)::所在地:Location','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GapCropRecord']=['id:UUID:PK:作物記録ID:Crop record ID','field_id:UUID:FK:圃場ID:Field ID','crop_name:VARCHAR(200):NOT NULL:作物名:Crop name','planting_date:DATE:NOT NULL:作付日:Planting date','harvest_date:DATE::収穫日:Harvest date','yield_kg:DECIMAL(10,2)::収量(kg):Yield (kg)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PesticideApplication']=['id:UUID:PK:農薬散布ID:Pesticide application ID','field_id:UUID:FK:圃場ID:Field ID','pesticide_name:VARCHAR(200):NOT NULL:農薬名:Pesticide name','application_date:DATE:NOT NULL:散布日:Application date','amount_l:DECIMAL(8,3)::散布量(L):Amount (L)','dilution_rate:VARCHAR(50)::希釈倍率:Dilution rate','operator_id:UUID:FK:作業者ID:Operator ID','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['FertilizerLog']=['id:UUID:PK:施肥記録ID:Fertilizer log ID','field_id:UUID:FK:圃場ID:Field ID','fertilizer_type:VARCHAR(200):NOT NULL:肥料種別:Fertilizer type','application_date:DATE:NOT NULL:施肥日:Application date','amount_kg:DECIMAL(8,2)::施肥量(kg):Amount (kg)','n_ratio:DECIMAL(5,2)::窒素比率:N ratio','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GapAuditResult']=['id:UUID:PK:GAP審査結果ID:GAP audit result ID','field_id:UUID:FK:圃場ID:Field ID','audit_date:DATE:NOT NULL:審査日:Audit date','certification_type:VARCHAR(100):NOT NULL:認証種別:Certification type','result:VARCHAR(50):NOT NULL:審査結果:Audit result','next_audit_date:DATE::次回審査日:Next audit date','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['FarmSimScenario']=['id:UUID:PK:シナリオID:Scenario ID','owner_id:UUID:FK:オーナーID:Owner ID','scenario_name:VARCHAR(200):NOT NULL:シナリオ名:Scenario name','target_year:INT:NOT NULL:対象年度:Target year','crop_mix_json:TEXT::作物組合せJSON:Crop mix JSON','base_area_ha:DECIMAL(8,2)::基準面積(ha):Base area (ha)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CropPlanItem']=['id:UUID:PK:作付計画ID:Crop plan item ID','scenario_id:UUID:FK:シナリオID:Scenario ID','crop_name:VARCHAR(200):NOT NULL:作物名:Crop name','planned_area_ha:DECIMAL(8,2):NOT NULL:計画面積(ha):Planned area (ha)','expected_yield_kg:DECIMAL(10,2)::期待収量(kg):Expected yield (kg)','unit_price_jpy:INTEGER::単価(円/kg):Unit price (JPY/kg)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['YieldForecast']=['id:UUID:PK:収量予測ID:Yield forecast ID','scenario_id:UUID:FK:シナリオID:Scenario ID','forecast_date:DATE:NOT NULL:予測日:Forecast date','model_type:VARCHAR(100):NOT NULL:モデル種別:Model type','predicted_yield_kg:DECIMAL(10,2):NOT NULL:予測収量(kg):Predicted yield (kg)','confidence:DECIMAL(5,2)::信頼度(%):Confidence (%)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CostSimEntry']=['id:UUID:PK:コストエントリID:Cost sim entry ID','scenario_id:UUID:FK:シナリオID:Scenario ID','cost_category:VARCHAR(100):NOT NULL:コストカテゴリ:Cost category','amount_jpy:INTEGER:NOT NULL:金額(円):Amount (JPY)','description:TEXT::説明:Description','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SubsidyApplication']=['id:UUID:PK:補助金申請ID:Subsidy application ID','scenario_id:UUID:FK:シナリオID:Scenario ID','subsidy_name:VARCHAR(200):NOT NULL:補助金名:Subsidy name','amount_jpy:INTEGER:NOT NULL:申請額(円):Application amount (JPY)','status:VARCHAR(50):DEFAULT \'draft\':ステータス:Status','submitted_at:TIMESTAMP::提出日時:Submitted at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['PhilTextCorpus']=['id:UUID:PK:テキストコーパスID:Text corpus ID','title:VARCHAR(300):NOT NULL:タイトル:Title','author:VARCHAR(200)::著者:Author','tradition:VARCHAR(100)::思想系統:Tradition','language:VARCHAR(50):NOT NULL:言語:Language','period:VARCHAR(100)::時代:Period','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PhilPassage']=['id:UUID:PK:テキスト節ID:Passage ID','corpus_id:UUID:FK:コーパスID:Corpus ID','passage_text:TEXT:NOT NULL:テキスト:Passage text','chapter:VARCHAR(100)::章:Chapter','verse:VARCHAR(100)::節:Verse','language:VARCHAR(50):NOT NULL:言語:Language','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PhilConcept']=['id:UUID:PK:哲学概念ID:Concept ID','concept_name:VARCHAR(200):NOT NULL:概念名:Concept name','tradition:VARCHAR(100)::思想系統:Tradition','definition_ja:TEXT::定義(日本語):Definition (JA)','definition_en:TEXT::定義(英語):Definition (EN)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PhilAnnotation']=['id:UUID:PK:注釈ID:Annotation ID','passage_id:UUID:FK:節ID:Passage ID','annotator_id:UUID:FK:注釈者ID:Annotator ID','annotation_text:TEXT:NOT NULL:注釈テキスト:Annotation text','tag:VARCHAR(100)::タグ:Tag','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PhilSearchResult']=['id:UUID:PK:検索結果ID:Search result ID','query_text:TEXT:NOT NULL:クエリ:Query text','passage_id:UUID:FK:節ID:Passage ID','similarity_score:DECIMAL(5,4)::類似度スコア:Similarity score','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['ThreeDModel']=['id:UUID:PK:3DモデルID:3D model ID','owner_id:UUID:FK:オーナーID:Owner ID','model_name:VARCHAR(200):NOT NULL:モデル名:Model name','file_format:VARCHAR(20):NOT NULL:ファイル形式:File format','file_url:TEXT:NOT NULL:ファイルURL:File URL','polygon_count:INTEGER::ポリゴン数:Polygon count','file_size_mb:DECIMAL(10,2)::ファイルサイズ(MB):File size (MB)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ThreeDModelVersion']=['id:UUID:PK:バージョンID:Version ID','model_id:UUID:FK:モデルID:Model ID','version_no:INTEGER:NOT NULL:バージョン番号:Version number','file_url:TEXT:NOT NULL:ファイルURL:File URL','change_notes:TEXT::変更備考:Change notes','created_by:UUID:FK:作成者ID:Created by','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['MaterialAsset']=['id:UUID:PK:素材アセットID:Material asset ID','owner_id:UUID:FK:オーナーID:Owner ID','material_name:VARCHAR(200):NOT NULL:素材名:Material name','texture_url:TEXT::テクスチャURL:Texture URL','material_type:VARCHAR(100)::素材種別:Material type','roughness:DECIMAL(4,2)::粗さ:Roughness','metalness:DECIMAL(4,2)::金属感:Metalness','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PrototypeComment']=['id:UUID:PK:コメントID:Prototype comment ID','model_id:UUID:FK:モデルID:Model ID','author_id:UUID:FK:作者ID:Author ID','comment_text:TEXT:NOT NULL:コメント:Comment text','position_json:TEXT::3D位置JSON:Position JSON','status:VARCHAR(50):DEFAULT \'open\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['RenderJob']=['id:UUID:PK:レンダリングジョブID:Render job ID','model_id:UUID:FK:モデルID:Model ID','render_type:VARCHAR(100):NOT NULL:レンダリング種別:Render type','output_url:TEXT::出力URL:Output URL','status:VARCHAR(50):DEFAULT \'queued\':ステータス:Status','completed_at:TIMESTAMP::完了日時:Completed at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['ContactCard']=['id:UUID:PK:名刺ID:Contact card ID','owner_id:UUID:FK:オーナーID:Owner ID','full_name:VARCHAR(200):NOT NULL:氏名:Full name','company:VARCHAR(200)::会社名:Company','title:VARCHAR(200)::役職:Title','email:VARCHAR(255)::メール:Email','phone:VARCHAR(50)::電話番号:Phone','scanned_image_url:TEXT::スキャン画像URL:Scanned image URL','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ContactOrg']=['id:UUID:PK:組織ID:Organization ID','org_name:VARCHAR(200):NOT NULL:組織名:Organization name','industry:VARCHAR(100)::業種:Industry','website:VARCHAR(300)::ウェブサイト:Website','address:TEXT::住所:Address','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ContactTag']=['id:UUID:PK:タグID:Tag ID','owner_id:UUID:FK:オーナーID:Owner ID','tag_name:VARCHAR(100):NOT NULL:タグ名:Tag name','color:VARCHAR(20)::色:Color','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['FollowUpTask']=['id:UUID:PK:フォローアップタスクID:Follow-up task ID','contact_id:UUID:FK:連絡先ID:Contact ID','owner_id:UUID:FK:オーナーID:Owner ID','due_date:DATE:NOT NULL:期限日:Due date','description:TEXT:NOT NULL:内容:Description','status:VARCHAR(50):DEFAULT \'pending\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ActivityLog']=['id:UUID:PK:活動ログID:Activity log ID','contact_id:UUID:FK:連絡先ID:Contact ID','owner_id:UUID:FK:オーナーID:Owner ID','activity_type:VARCHAR(100):NOT NULL:活動種別:Activity type','summary:TEXT::概要:Summary','occurred_at:TIMESTAMP:NOT NULL:発生日時:Occurred at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['InboundTransaction']=['id:UUID:PK:インバウンド決済ID:Inbound transaction ID','merchant_id:UUID:FK:加盟店ID:Merchant ID','amount:DECIMAL(12,2):NOT NULL:金額:Amount','currency:VARCHAR(10):NOT NULL:通貨:Currency','amount_jpy:DECIMAL(12,2):NOT NULL:円換算額:Amount (JPY)','payment_method:VARCHAR(100):NOT NULL:決済手段:Payment method','status:VARCHAR(50):DEFAULT \'pending\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TaxFreeRecord']=['id:UUID:PK:免税記録ID:Tax-free record ID','transaction_id:UUID:FK:決済ID:Transaction ID','passport_no:VARCHAR(100):NOT NULL:パスポート番号:Passport number','nationality:VARCHAR(100)::国籍:Nationality','tax_amount_jpy:INTEGER:NOT NULL:免税額(円):Tax amount (JPY)','receipt_url:TEXT::領収書URL:Receipt URL','status:VARCHAR(50):DEFAULT \'issued\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PassportVerify']=['id:UUID:PK:パスポート確認ID:Passport verify ID','customer_id:UUID:FK:顧客ID:Customer ID','passport_no:VARCHAR(100):NOT NULL:パスポート番号:Passport number','country_code:VARCHAR(10):NOT NULL:国コード:Country code','expiry_date:DATE:NOT NULL:有効期限:Expiry date','verified_at:TIMESTAMP:NOT NULL:確認日時:Verified at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CurrencyRate']=['id:UUID:PK:為替レートID:Currency rate ID','base_currency:VARCHAR(10):NOT NULL:基準通貨:Base currency','target_currency:VARCHAR(10):NOT NULL:対象通貨:Target currency','rate:DECIMAL(12,6):NOT NULL:レート:Rate','recorded_at:TIMESTAMP:NOT NULL:記録日時:Recorded at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TaxFiling']=['id:UUID:PK:税務申告ID:Tax filing ID','merchant_id:UUID:FK:加盟店ID:Merchant ID','period_month:VARCHAR(10):NOT NULL:対象月:Period month','total_tax_refund_jpy:INTEGER:NOT NULL:合計還付税額(円):Total tax refund (JPY)','status:VARCHAR(50):DEFAULT \'draft\':ステータス:Status','submitted_at:TIMESTAMP::提出日時:Submitted at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['LabAnimalRecord']=['id:UUID:PK:実験動物記録ID:Lab animal record ID','species:VARCHAR(100):NOT NULL:種:Species','strain:VARCHAR(200)::系統:Strain','animal_id_code:VARCHAR(100):UNIQUE NOT NULL:個体識別コード:Animal ID code','birth_date:DATE::誕生日:Birth date','sex:VARCHAR(20):NOT NULL:性別:Sex','status:VARCHAR(50):DEFAULT \'active\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['AnimalCageUnit']=['id:UUID:PK:ケージユニットID:Cage unit ID','cage_code:VARCHAR(100):UNIQUE NOT NULL:ケージコード:Cage code','room_location:VARCHAR(200):NOT NULL:室内場所:Room location','capacity:INTEGER:NOT NULL:収容数:Capacity','current_count:INTEGER:DEFAULT 0:現在収容数:Current count','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ExperimentProto']=['id:UUID:PK:実験プロトコルID:Experiment protocol ID','pi_id:UUID:FK:研究責任者ID:PI ID','protocol_title:VARCHAR(300):NOT NULL:プロトコル名:Protocol title','species:VARCHAR(100):NOT NULL:使用種:Species','animal_count:INTEGER:NOT NULL:使用数:Animal count','ethics_status:VARCHAR(50):DEFAULT \'pending\':倫理審査状況:Ethics status','approved_at:TIMESTAMP::承認日時:Approved at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['EthicsApplication']=['id:UUID:PK:倫理申請ID:Ethics application ID','protocol_id:UUID:FK:プロトコルID:Protocol ID','applicant_id:UUID:FK:申請者ID:Applicant ID','submitted_at:TIMESTAMP:NOT NULL:提出日時:Submitted at','review_result:VARCHAR(50)::審査結果:Review result','reviewed_at:TIMESTAMP::審査日時:Reviewed at','notes:TEXT::備考:Notes','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['AnimalObservation']=['id:UUID:PK:観察記録ID:Animal observation ID','animal_id:UUID:FK:動物ID:Animal ID','observer_id:UUID:FK:観察者ID:Observer ID','observed_at:TIMESTAMP:NOT NULL:観察日時:Observed at','body_weight_g:DECIMAL(8,2)::体重(g):Body weight (g)','health_status:VARCHAR(100)::健康状態:Health status','notes:TEXT::備考:Notes','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['ShareVehicle']=['id:UUID:PK:シェア車両ID:Share vehicle ID','fleet_id:UUID:FK:フリートID:Fleet ID','license_plate:VARCHAR(50):UNIQUE NOT NULL:ナンバープレート:License plate','vehicle_type:VARCHAR(100):NOT NULL:車種:Vehicle type','status:VARCHAR(50):DEFAULT \'available\':ステータス:Status','location_lat:DECIMAL(9,6)::緯度:Latitude','location_lng:DECIMAL(9,6)::経度:Longitude','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ShareReservation']=['id:UUID:PK:予約ID:Reservation ID','vehicle_id:UUID:FK:車両ID:Vehicle ID','user_id:UUID:FK:ユーザーID:User ID','start_at:TIMESTAMP:NOT NULL:開始日時:Start at','end_at:TIMESTAMP:NOT NULL:終了日時:End at','status:VARCHAR(50):DEFAULT \'confirmed\':ステータス:Status','total_fee_jpy:INTEGER::料金(円):Total fee (JPY)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ShareKey']=['id:UUID:PK:スマートキーID:Smart key ID','vehicle_id:UUID:FK:車両ID:Vehicle ID','key_token:TEXT:UNIQUE NOT NULL:キートークン:Key token','valid_from:TIMESTAMP:NOT NULL:有効開始:Valid from','valid_until:TIMESTAMP:NOT NULL:有効終了:Valid until','revoked:BOOLEAN:DEFAULT FALSE:失効:Revoked','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['VehicleService']=['id:UUID:PK:整備記録ID:Vehicle service ID','vehicle_id:UUID:FK:車両ID:Vehicle ID','service_type:VARCHAR(100):NOT NULL:整備種別:Service type','scheduled_at:DATE:NOT NULL:予定日:Scheduled at','completed_at:TIMESTAMP::完了日時:Completed at','cost_jpy:INTEGER::費用(円):Cost (JPY)','notes:TEXT::備考:Notes','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['FleetUtilization']=['id:UUID:PK:稼働率記録ID:Fleet utilization ID','vehicle_id:UUID:FK:車両ID:Vehicle ID','period_date:DATE:NOT NULL:対象日:Period date','usage_hours:DECIMAL(5,2)::稼働時間:Usage hours','revenue_jpy:INTEGER::売上(円):Revenue (JPY)','distance_km:DECIMAL(10,2)::走行距離(km):Distance (km)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['InsuranceClaim']=['id:UUID:PK:保険金請求ID:Insurance claim ID','policy_id:UUID:FK:保険証券ID:Policy ID','claimant_id:UUID:FK:請求者ID:Claimant ID','claim_date:DATE:NOT NULL:請求日:Claim date','incident_date:DATE:NOT NULL:事故日:Incident date','claim_type:VARCHAR(100):NOT NULL:請求種別:Claim type','claimed_amount_jpy:INTEGER:NOT NULL:請求額(円):Claimed amount (JPY)','status:VARCHAR(50):DEFAULT \'received\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ClaimDocument']=['id:UUID:PK:請求書類ID:Claim document ID','claim_id:UUID:FK:請求ID:Claim ID','doc_type:VARCHAR(100):NOT NULL:書類種別:Document type','file_url:TEXT:NOT NULL:ファイルURL:File URL','ocr_result:TEXT::OCR結果:OCR result','verified:BOOLEAN:DEFAULT FALSE:確認済:Verified','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['FraudAlert']=['id:UUID:PK:不正アラートID:Fraud alert ID','claim_id:UUID:FK:請求ID:Claim ID','fraud_score:DECIMAL(5,3):NOT NULL:不正スコア:Fraud score','alert_reason:TEXT:NOT NULL:アラート理由:Alert reason','status:VARCHAR(50):DEFAULT \'open\':ステータス:Status','reviewed_by:UUID:FK:確認者ID:Reviewed by','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ClaimApproval']=['id:UUID:PK:承認ID:Claim approval ID','claim_id:UUID:FK:請求ID:Claim ID','approver_id:UUID:FK:承認者ID:Approver ID','approved_amount_jpy:INTEGER:NOT NULL:承認額(円):Approved amount (JPY)','decision:VARCHAR(50):NOT NULL:決定:Decision','decision_notes:TEXT::決定備考:Decision notes','decided_at:TIMESTAMP:NOT NULL:決定日時:Decided at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PolicyHolder']=['id:UUID:PK:保険契約者ID:Policy holder ID','full_name:VARCHAR(200):NOT NULL:氏名:Full name','birth_date:DATE:NOT NULL:生年月日:Birth date','address:TEXT::住所:Address','policy_no:VARCHAR(100):UNIQUE NOT NULL:証券番号:Policy number','policy_type:VARCHAR(100):NOT NULL:保険種別:Policy type','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['LandParcel']=['id:UUID:PK:土地ID:Land parcel ID','owner_id:UUID:FK:所有者ID:Owner ID','land_number:VARCHAR(100):NOT NULL:地番:Land number','municipality:VARCHAR(200):NOT NULL:市区町村:Municipality','area_sqm:DECIMAL(12,2):NOT NULL:面積(㎡):Area (sqm)','land_use:VARCHAR(100)::地目:Land use','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PropertyCert']=['id:UUID:PK:不動産謄本ID:Property cert ID','parcel_id:UUID:FK:土地ID:Parcel ID','cert_type:VARCHAR(100):NOT NULL:謄本種別:Certificate type','issued_at:DATE:NOT NULL:発行日:Issued at','file_url:TEXT::謄本ファイルURL:Certificate file URL','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ParcelValuation']=['id:UUID:PK:土地評価ID:Parcel valuation ID','parcel_id:UUID:FK:土地ID:Parcel ID','valuation_date:DATE:NOT NULL:評価日:Valuation date','road_value_jpy:INTEGER::路線価(円/㎡):Road value (JPY/sqm)','assessed_value_jpy:BIGINT::評価額(円):Assessed value (JPY)','method:VARCHAR(100)::評価方式:Valuation method','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['RegistrationDoc']=['id:UUID:PK:登記書類ID:Registration doc ID','parcel_id:UUID:FK:土地ID:Parcel ID','doc_type:VARCHAR(100):NOT NULL:書類種別:Document type','prepared_by:UUID:FK:作成者ID:Prepared by','status:VARCHAR(50):DEFAULT \'draft\':ステータス:Status','submitted_at:TIMESTAMP::提出日時:Submitted at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['LandOwner']=['id:UUID:PK:土地所有者ID:Land owner ID','full_name:VARCHAR(200):NOT NULL:氏名:Full name','owner_type:VARCHAR(50):NOT NULL:所有者種別:Owner type','address:TEXT::住所:Address','registration_no:VARCHAR(100)::登記番号:Registration number','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

// Field preset entities
ENTITY_COLUMNS['GapComplianceField']=['id:UUID:PK:GAP準拠圃場ID:GAP compliance field ID','field_name:VARCHAR(200):NOT NULL:圃場名:Field name','compliance_score:DECIMAL(5,2)::準拠スコア:Compliance score','gap_standard:VARCHAR(100):NOT NULL:GAP規格:GAP standard','last_check:DATE::最終確認日:Last check date','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PesticideRiskScore']=['id:UUID:PK:農薬リスクスコアID:Pesticide risk score ID','field_id:UUID:FK:圃場ID:Field ID','pesticide_name:VARCHAR(200):NOT NULL:農薬名:Pesticide name','risk_level:VARCHAR(50):NOT NULL:リスクレベル:Risk level','score:DECIMAL(5,3):NOT NULL:スコア:Score','evaluated_at:DATE:NOT NULL:評価日:Evaluated at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GapMilestone']=['id:UUID:PK:GAPマイルストーンID:GAP milestone ID','farm_id:UUID:FK:農場ID:Farm ID','milestone_name:VARCHAR(200):NOT NULL:マイルストーン名:Milestone name','target_date:DATE:NOT NULL:目標日:Target date','status:VARCHAR(50):DEFAULT \'pending\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CertAuditLog']=['id:UUID:PK:認証監査ログID:Cert audit log ID','farm_id:UUID:FK:農場ID:Farm ID','audit_type:VARCHAR(100):NOT NULL:監査種別:Audit type','auditor:VARCHAR(200)::審査員:Auditor','result:VARCHAR(50):NOT NULL:結果:Result','audit_date:DATE:NOT NULL:監査日:Audit date','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TraceabilityReport']=['id:UUID:PK:トレーサビリティ報告ID:Traceability report ID','crop_batch_id:UUID:FK:作物バッチID:Crop batch ID','report_type:VARCHAR(100):NOT NULL:報告種別:Report type','generated_at:TIMESTAMP:NOT NULL:生成日時:Generated at','cert_url:TEXT::証明書URL:Certificate URL','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['FarmSimModel']=['id:UUID:PK:農業シミュレーションモデルID:Farm sim model ID','model_name:VARCHAR(200):NOT NULL:モデル名:Model name','model_type:VARCHAR(100):NOT NULL:モデル種別:Model type','parameters_json:TEXT::パラメータJSON:Parameters JSON','accuracy_pct:DECIMAL(5,2)::精度(%):Accuracy (%)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['WeatherDataset']=['id:UUID:PK:気象データセットID:Weather dataset ID','station_name:VARCHAR(200):NOT NULL:観測所名:Station name','region:VARCHAR(200)::地域:Region','recorded_date:DATE:NOT NULL:記録日:Recorded date','temp_avg_c:DECIMAL(6,2)::平均気温(℃):Avg temperature (℃)','rainfall_mm:DECIMAL(8,2)::降水量(mm):Rainfall (mm)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['MarketPriceHistory']=['id:UUID:PK:市場価格履歴ID:Market price history ID','crop_name:VARCHAR(200):NOT NULL:作物名:Crop name','market_name:VARCHAR(200)::市場名:Market name','price_date:DATE:NOT NULL:価格日:Price date','price_jpy_kg:DECIMAL(10,2):NOT NULL:価格(円/kg):Price (JPY/kg)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CropGrowthSim']=['id:UUID:PK:作物成長シミュレーションID:Crop growth sim ID','model_id:UUID:FK:モデルID:Model ID','crop_name:VARCHAR(200):NOT NULL:作物名:Crop name','growth_stage:VARCHAR(100):NOT NULL:生育ステージ:Growth stage','predicted_harvest:DATE::収穫予測日:Predicted harvest date','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['BusinessScenario']=['id:UUID:PK:経営シナリオID:Business scenario ID','farm_id:UUID:FK:農場ID:Farm ID','scenario_name:VARCHAR(200):NOT NULL:シナリオ名:Scenario name','projected_revenue_jpy:BIGINT::見込売上(円):Projected revenue (JPY)','projected_cost_jpy:BIGINT::見込コスト(円):Projected cost (JPY)','risk_level:VARCHAR(50)::リスクレベル:Risk level','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['PhilCorpusIndex']=['id:UUID:PK:コーパスインデックスID:Corpus index ID','corpus_id:UUID:FK:コーパスID:Corpus ID','embedding_model:VARCHAR(200):NOT NULL:埋め込みモデル:Embedding model','indexed_at:TIMESTAMP:NOT NULL:インデックス日時:Indexed at','vector_count:INTEGER::ベクター数:Vector count','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['SemanticCluster']=['id:UUID:PK:セマンティッククラスタID:Semantic cluster ID','cluster_name:VARCHAR(200):NOT NULL:クラスタ名:Cluster name','theme_ja:TEXT::テーマ(日本語):Theme (JA)','theme_en:TEXT::テーマ(英語):Theme (EN)','member_count:INTEGER:DEFAULT 0:メンバー数:Member count','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ConceptRelation']=['id:UUID:PK:概念関係ID:Concept relation ID','concept_a_id:UUID:FK:概念AID:Concept A ID','concept_b_id:UUID:FK:概念BID:Concept B ID','relation_type:VARCHAR(100):NOT NULL:関係種別:Relation type','strength:DECIMAL(5,3)::強度:Strength','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TranslationPair']=['id:UUID:PK:翻訳ペアID:Translation pair ID','passage_id:UUID:FK:節ID:Passage ID','source_lang:VARCHAR(20):NOT NULL:原語:Source language','target_lang:VARCHAR(20):NOT NULL:訳語:Target language','translation_text:TEXT:NOT NULL:翻訳文:Translation text','translator:VARCHAR(200)::翻訳者:Translator','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['CitationNetwork']=['id:UUID:PK:引用ネットワークID:Citation network ID','citing_passage_id:UUID:FK:引用節ID:Citing passage ID','cited_passage_id:UUID:FK:被引用節ID:Cited passage ID','relation_type:VARCHAR(100):NOT NULL:関係種別:Relation type','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['ThreeDDesignProject']=['id:UUID:PK:3D設計プロジェクトID:3D design project ID','owner_id:UUID:FK:オーナーID:Owner ID','project_name:VARCHAR(200):NOT NULL:プロジェクト名:Project name','description:TEXT::説明:Description','status:VARCHAR(50):DEFAULT \'active\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GeometryValidation']=['id:UUID:PK:ジオメトリ検証ID:Geometry validation ID','model_id:UUID:FK:モデルID:Model ID','validated_at:TIMESTAMP:NOT NULL:検証日時:Validated at','error_count:INTEGER:DEFAULT 0:エラー数:Error count','warning_count:INTEGER:DEFAULT 0:警告数:Warning count','report_json:TEXT::検証レポートJSON:Report JSON','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['MaterialRecommend']=['id:UUID:PK:素材推薦ID:Material recommend ID','model_id:UUID:FK:モデルID:Model ID','recommended_material:VARCHAR(200):NOT NULL:推薦素材:Recommended material','reason_ja:TEXT::理由(日本語):Reason (JA)','reason_en:TEXT::理由(英語):Reason (EN)','confidence:DECIMAL(5,3)::信頼度:Confidence','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PrintJobParam']=['id:UUID:PK:印刷ジョブパラメータID:Print job param ID','model_id:UUID:FK:モデルID:Model ID','printer_type:VARCHAR(100):NOT NULL:プリンター種別:Printer type','layer_height_mm:DECIMAL(5,3)::レイヤー高さ(mm):Layer height (mm)','infill_pct:INTEGER::インフィル(%):Infill (%)','support_type:VARCHAR(100)::サポート種別:Support type','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['DesignIteration']=['id:UUID:PK:デザインイテレーションID:Design iteration ID','project_id:UUID:FK:プロジェクトID:Project ID','iteration_no:INTEGER:NOT NULL:イテレーション番号:Iteration number','changes_summary:TEXT::変更サマリ:Changes summary','feedback_status:VARCHAR(50):DEFAULT \'pending\':フィードバックステータス:Feedback status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['ContactNode']=['id:UUID:PK:コンタクトノードID:Contact node ID','owner_id:UUID:FK:オーナーID:Owner ID','full_name:VARCHAR(200):NOT NULL:氏名:Full name','company:VARCHAR(200)::会社名:Company','role:VARCHAR(200)::役職:Role','email:VARCHAR(255)::メール:Email','relationship_type:VARCHAR(100)::関係種別:Relationship type','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['RelationScore']=['id:UUID:PK:関係スコアID:Relation score ID','contact_a:UUID:FK:コンタクトAID:Contact A ID','contact_b:UUID:FK:コンタクトBID:Contact B ID','score:DECIMAL(5,3):NOT NULL:スコア:Score','interaction_count:INTEGER:DEFAULT 0:インタラクション数:Interaction count','last_contact:DATE::最終連絡日:Last contact date','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['NetworkGroup']=['id:UUID:PK:ネットワークグループID:Network group ID','owner_id:UUID:FK:オーナーID:Owner ID','group_name:VARCHAR(200):NOT NULL:グループ名:Group name','description:TEXT::説明:Description','member_count:INTEGER:DEFAULT 0:メンバー数:Member count','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['NetworkFollowUpPlan']=['id:UUID:PK:フォローアッププランID:Follow-up plan ID','contact_id:UUID:FK:コンタクトID:Contact ID','plan_type:VARCHAR(100):NOT NULL:プラン種別:Plan type','scheduled_date:DATE:NOT NULL:予定日:Scheduled date','action_ja:TEXT::アクション(日本語):Action (JA)','action_en:TEXT::アクション(英語):Action (EN)','completed:BOOLEAN:DEFAULT FALSE:完了:Completed','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['InteractionHistory']=['id:UUID:PK:インタラクション履歴ID:Interaction history ID','contact_id:UUID:FK:コンタクトID:Contact ID','interaction_type:VARCHAR(100):NOT NULL:インタラクション種別:Interaction type','summary:TEXT::概要:Summary','occurred_at:TIMESTAMP:NOT NULL:発生日時:Occurred at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['InboundPayAnalysis']=['id:UUID:PK:インバウンド決済分析ID:Inbound pay analysis ID','merchant_id:UUID:FK:加盟店ID:Merchant ID','period_month:VARCHAR(10):NOT NULL:対象月:Period month','total_amount_jpy:BIGINT::合計額(円):Total amount (JPY)','transaction_count:INTEGER::件数:Transaction count','top_currency:VARCHAR(10)::主要通貨:Top currency','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['MultiCurrencyTx']=['id:UUID:PK:多通貨決済ID:Multi-currency transaction ID','merchant_id:UUID:FK:加盟店ID:Merchant ID','amount:DECIMAL(12,2):NOT NULL:金額:Amount','currency:VARCHAR(10):NOT NULL:通貨:Currency','exchange_rate:DECIMAL(12,6):NOT NULL:為替レート:Exchange rate','amount_jpy:DECIMAL(12,2):NOT NULL:円換算額:Amount (JPY)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['TaxFreeOptimize']=['id:UUID:PK:免税最適化ID:Tax-free optimize ID','merchant_id:UUID:FK:加盟店ID:Merchant ID','recommendation_ja:TEXT::推薦内容(日本語):Recommendation (JA)','recommendation_en:TEXT::推薦内容(英語):Recommendation (EN)','potential_savings_jpy:INTEGER::節約見込み(円):Potential savings (JPY)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['InboundFraudSignal']=['id:UUID:PK:不正シグナルID:Inbound fraud signal ID','transaction_id:UUID:FK:決済ID:Transaction ID','signal_type:VARCHAR(100):NOT NULL:シグナル種別:Signal type','risk_score:DECIMAL(5,3):NOT NULL:リスクスコア:Risk score','action_taken:VARCHAR(100)::実施アクション:Action taken','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ConsumptionForecast']=['id:UUID:PK:消費予測ID:Consumption forecast ID','merchant_id:UUID:FK:加盟店ID:Merchant ID','forecast_period:VARCHAR(20):NOT NULL:予測期間:Forecast period','expected_amount_jpy:BIGINT:NOT NULL:予測額(円):Expected amount (JPY)','confidence_pct:DECIMAL(5,2)::信頼度(%):Confidence (%)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['AnimalResearchUnit']=['id:UUID:PK:動物研究ユニットID:Animal research unit ID','unit_name:VARCHAR(200):NOT NULL:ユニット名:Unit name','species:VARCHAR(100):NOT NULL:種:Species','experiment_goal:TEXT::実験目的:Experiment goal','start_date:DATE::開始日:Start date','status:VARCHAR(50):DEFAULT \'active\':ステータス:Status','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['GenotypePhenotype']=['id:UUID:PK:遺伝子型表現型ID:Genotype phenotype ID','animal_id:UUID:FK:動物ID:Animal ID','genotype:VARCHAR(300)::遺伝子型:Genotype','phenotype_description:TEXT::表現型説明:Phenotype description','assessed_at:DATE:NOT NULL:評価日:Assessed at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['BehaviorDataset']=['id:UUID:PK:行動データセットID:Behavior dataset ID','unit_id:UUID:FK:ユニットID:Unit ID','behavior_type:VARCHAR(100):NOT NULL:行動種別:Behavior type','recording_url:TEXT::記録URL:Recording URL','analysis_result:TEXT::解析結果:Analysis result','recorded_at:TIMESTAMP:NOT NULL:記録日時:Recorded at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['EthicsReviewForm']=['id:UUID:PK:倫理審査フォームID:Ethics review form ID','protocol_id:UUID:FK:プロトコルID:Protocol ID','form_version:VARCHAR(50):NOT NULL:フォームバージョン:Form version','content_json:TEXT::内容JSON:Content JSON','submitted_at:TIMESTAMP:NOT NULL:提出日時:Submitted at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['WelfareAlert']=['id:UUID:PK:福祉アラートID:Welfare alert ID','animal_id:UUID:FK:動物ID:Animal ID','alert_type:VARCHAR(100):NOT NULL:アラート種別:Alert type','severity:VARCHAR(50):NOT NULL:重大度:Severity','description:TEXT:NOT NULL:説明:Description','resolved_at:TIMESTAMP::解決日時:Resolved at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['MobilityVehicle']=['id:UUID:PK:モビリティ車両ID:Mobility vehicle ID','vehicle_type:VARCHAR(100):NOT NULL:車種:Vehicle type','license_plate:VARCHAR(50):UNIQUE NOT NULL:ナンバープレート:License plate','status:VARCHAR(50):DEFAULT \'available\':ステータス:Status','lat:DECIMAL(9,6)::緯度:Latitude','lng:DECIMAL(9,6)::経度:Longitude','battery_pct:INTEGER::バッテリー(%):Battery (%)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['UsagePattern']=['id:UUID:PK:利用パターンID:Usage pattern ID','vehicle_id:UUID:FK:車両ID:Vehicle ID','day_of_week:INTEGER:NOT NULL:曜日:Day of week','hour_of_day:INTEGER:NOT NULL:時間帯:Hour of day','avg_duration_min:DECIMAL(8,2)::平均利用時間(分):Avg duration (min)','demand_score:DECIMAL(5,3)::需要スコア:Demand score','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['DemandForecast']=['id:UUID:PK:需要予測ID:Demand forecast ID','zone_id:UUID:FK:ゾーンID:Zone ID','forecast_datetime:TIMESTAMP:NOT NULL:予測日時:Forecast datetime','predicted_demand:DECIMAL(8,2):NOT NULL:予測需要:Predicted demand','model_version:VARCHAR(50)::モデルバージョン:Model version','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['PricingModel']=['id:UUID:PK:価格モデルID:Pricing model ID','model_name:VARCHAR(200):NOT NULL:モデル名:Model name','base_rate_jpy:INTEGER:NOT NULL:基本料金(円/時):Base rate (JPY/h)','peak_multiplier:DECIMAL(4,2):DEFAULT 1.0:ピーク倍率:Peak multiplier','is_active:BOOLEAN:DEFAULT TRUE:有効:Active','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ServiceZone']=['id:UUID:PK:サービスゾーンID:Service zone ID','zone_name:VARCHAR(200):NOT NULL:ゾーン名:Zone name','boundary_json:TEXT::境界JSON:Boundary JSON','vehicle_quota:INTEGER:NOT NULL:配備台数:Vehicle quota','revenue_score:DECIMAL(5,3)::収益スコア:Revenue score','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['ClaimAnalysis']=['id:UUID:PK:請求分析ID:Claim analysis ID','claim_id:UUID:FK:請求ID:Claim ID','analysis_type:VARCHAR(100):NOT NULL:分析種別:Analysis type','finding_ja:TEXT::所見(日本語):Finding (JA)','finding_en:TEXT::所見(英語):Finding (EN)','confidence:DECIMAL(5,3)::信頼度:Confidence','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['FraudScore']=['id:UUID:PK:不正スコアID:Fraud score ID','claim_id:UUID:FK:請求ID:Claim ID','score:DECIMAL(5,3):NOT NULL:スコア:Score','model_version:VARCHAR(100):NOT NULL:モデルバージョン:Model version','features_json:TEXT::特徴JSON:Features JSON','scored_at:TIMESTAMP:NOT NULL:スコア日時:Scored at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ReinsuranceRisk']=['id:UUID:PK:再保険リスクID:Reinsurance risk ID','policy_id:UUID:FK:証券ID:Policy ID','risk_tier:VARCHAR(50):NOT NULL:リスクティア:Risk tier','exposure_jpy:BIGINT:NOT NULL:エクスポージャー(円):Exposure (JPY)','reinsurer:VARCHAR(200)::再保険会社:Reinsurer','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['RegComplianceReport']=['id:UUID:PK:規制準拠報告ID:Regulatory compliance report ID','insurer_id:UUID:FK:保険会社ID:Insurer ID','report_period:VARCHAR(20):NOT NULL:報告期間:Report period','regulator:VARCHAR(200):NOT NULL:規制機関:Regulator','status:VARCHAR(50):DEFAULT \'draft\':ステータス:Status','submitted_at:TIMESTAMP::提出日時:Submitted at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ClaimFastTrack']=['id:UUID:PK:ファストトラックID:Claim fast track ID','claim_id:UUID:FK:請求ID:Claim ID','eligible:BOOLEAN:NOT NULL:対象:Eligible','reason_ja:TEXT::理由(日本語):Reason (JA)','reason_en:TEXT::理由(英語):Reason (EN)','payment_estimate_jpy:INTEGER::支払見込み(円):Payment estimate (JPY)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];

ENTITY_COLUMNS['LandValuationAI']=['id:UUID:PK:土地評価AIレコードID:Land valuation AI record ID','parcel_id:UUID:FK:土地ID:Parcel ID','model_name:VARCHAR(200):NOT NULL:モデル名:Model name','predicted_value_jpy:BIGINT:NOT NULL:AI予測評価額(円):AI predicted value (JPY)','confidence_pct:DECIMAL(5,2)::信頼度(%):Confidence (%)','evaluated_at:DATE:NOT NULL:評価日:Evaluated at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['RoadValueMap']=['id:UUID:PK:路線価マップID:Road value map ID','municipality:VARCHAR(200):NOT NULL:市区町村:Municipality','road_segment:VARCHAR(300):NOT NULL:路線区分:Road segment','road_value_jpy:INTEGER:NOT NULL:路線価(円/㎡):Road value (JPY/sqm)','fiscal_year:INT:NOT NULL:年度:Fiscal year','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['InheritanceSimulation']=['id:UUID:PK:相続税シミュレーションID:Inheritance simulation ID','parcel_id:UUID:FK:土地ID:Parcel ID','scenario_name:VARCHAR(200):NOT NULL:シナリオ名:Scenario name','total_estate_jpy:BIGINT:NOT NULL:遺産総額(円):Total estate (JPY)','tax_amount_jpy:BIGINT::相続税額(円):Tax amount (JPY)','deduction_applied:BOOLEAN:DEFAULT FALSE:控除適用:Deduction applied','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['RegistrationFlow']=['id:UUID:PK:登記フローID:Registration flow ID','parcel_id:UUID:FK:土地ID:Parcel ID','flow_type:VARCHAR(100):NOT NULL:フロー種別:Flow type','current_step:VARCHAR(100)::現在ステップ:Current step','completion_pct:INTEGER:DEFAULT 0:完了率(%):Completion (%)','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['OwnershipRisk']=['id:UUID:PK:権利リスクID:Ownership risk ID','parcel_id:UUID:FK:土地ID:Parcel ID','risk_type:VARCHAR(100):NOT NULL:リスク種別:Risk type','risk_description:TEXT:NOT NULL:リスク説明:Risk description','severity:VARCHAR(50):NOT NULL:重大度:Severity','detected_at:DATE:NOT NULL:検出日:Detected at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
ENTITY_COLUMNS['ClaimLog']=['id:UUID:PK:請求監査ログID:Claim audit log ID','claim_id:UUID:FK:請求ID:Claim ID','action:VARCHAR(100):NOT NULL:操作:Action','actor_id:UUID:FK:操作者ID:Actor ID','changed_fields:TEXT::変更フィールド:Changed fields','occurred_at:TIMESTAMP:NOT NULL DEFAULT NOW():発生日時:Occurred at','created_at:TIMESTAMP:NOT NULL DEFAULT NOW():作成日時:Created at'];
