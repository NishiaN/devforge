...
- **CDNライブラリ更新の再検討（v9.40でHOLD）**: marked 12.0.2/mermaid 10.9.1。cdnjs配信上限(marked16.3/mermaid11.12)＋mermaid v11破壊的変更(`mermaid.default.*`API＋ラベル互換修正11.13.0がcdnjs未達)で現在HOLD。**cdnjsが最新配信するか、jsDelivr等でmermaid 11.13+へ移行＋initialize/render呼び出し書換え＋回帰テストを許容できる時に着手**。integrity(SRI)ハッシュ再計算も必須。判断根拠=docs/plans/v940-freshness-update-plan.md
- **GitHub Actions最新化の余地**: v9.40でv4→v5世代に更新済だが、真の最新はcheckout/upload@v7・setup-node@v6。v5は安全な現行世代として選択。さらに上げる場合はNode24ランナー前提とcodecov等の追加破壊的変更を要確認
- **launcher.js ja/en 圧縮**（393KB）: サイズ逼迫時（現在~541KB余裕で優先度低）。v9.22の共有ブロック方式の全面展開
- **7アンチパターン明示チェックリスト**: 参考資料の残り題材（既存設計と重複多く保留中）
- **why_ja 350B超過3件のトリム**: Netlify Functions×2 + Python logging（387〜530B）。機能影響なし・低優先
- 完了済: ~~features空84件充填~~(v9.39) | ~~コピーライト刷新+版数統一~~(v9.39a) | ~~techdb鮮度刷新538→548~~(v9.40) | ~~techdb選択の影響機能照査(SSR修正)~~(v9.40) | ~~中身の鮮度Tier1-5(AIモデル/料金/GH Actions/MCP/機能カウント/日付/doc数)~~(v9.40)

**次回セッションの開始点**: 鮮度の主要陳腐化は解消済（AIモデル・カウント・日付・techdb）。この先送りリストから再評価（着手前に価値を実測 — v9.26/v9.29の教訓）。新規の価値実測（生成物のgrep掃引・実バンドルE2E）から着手候補を探すのが有効。CDN更新は外部条件(cdnjs配信/mermaid移行許容)が整った時。
