...
- **漢字のみ残留のEN掃引**（筆頭候補）: v9.31のかな検出（`[぀-ゟ゠-ヿ]{2,}`）は漢字のみの行（例: `5分/5m`、`高`/`低` セル、`品質劣化`）を拾えない。エージェント報告で隣接分は修正済みだが全数掃引は未実施。検出は「CJK統合漢字のみ行 − 中国語共有の技術語（例: 監査/認証等は文脈次第）」の判別が必要で、かなより難しい — サンプリング+目視が現実的
- **launcher.js ja/en 圧縮**（393KB）: サイズ逼迫時に着手（現在575KB余裕）。v9.22で確立した共有ブロック方式の全面展開
- **7アンチパターン明示チェックリスト**: 参考資料の残り題材（既存設計と重複多く保留中）
- **why_ja 350B超過3件のトリム**: Netlify Functions×2 + Python logging（387〜530B）。機能影響なし・低優先
- 完了済: ~~scope_out AI可視化~~(v9.30) | ~~scope_out why文検証~~(v9.29) | ~~doc番号レジストリ~~(v9.28) | ~~EN日本語根絶(かな)~~(v9.31)
