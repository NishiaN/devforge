...
- **features空のフィールドプリセット84件の充填**（新・筆頭候補）: eng5_/sci5_/cce5_ 等の後期バッチ84件は features:[] のままで、実アプリではmvp_features がUPP-Hの衛生機能のみになる（v9.38実測で発見）。ドメイン固有のMVP機能4〜6件（ja+en）を各プリセットに追加する。84件×バイリンガル＝並列修正の鉄則（ソースファイル排他分担）が適用できる規模。対象リストは `Object.keys(PR_FIELD).filter(k=>!(PR_FIELD[k].features||[]).length)` で再取得
...
- 完了済: ~~scope_out AI可視化~~(v9.30) | ~~scope_out why文検証~~(v9.29) | ~~doc番号レジストリ~~(v9.28) | ~~EN日本語根絶(かな)~~(v9.31) | ~~漢字のみEN掃引~~(v9.32) | ~~npm testフレーク解消~~(v9.33) | ~~全プリセット完全性掃引~~(v9.34) | ~~列挙生成検査の横展開(xref/Mermaid/フェンス)~~(v9.35) | ~~回答キー誤用の静的検査~~(v9.36) | ~~scale伝播の実挙動確認~~(v9.37) | ~~ハーネス注入キー整合(アプリ経路化+UPP-H)~~(v9.38)

**次回セッションの開始点**: この先送りリストの筆頭から再評価（着手前に価値を実測 — v9.26/v9.29の教訓）。features空84件は「1件サンプル充填→compat-check/sweepで影響確認→84件並列展開」の順が安全。
