...
**最終更新**: 2026-07-06 | **現在値**: v9.40 / 7525 tests / ~5959KB / 548 techdb / 116 templates (6500KB上限) / 116 launcher templates / 28 pillars / 227+ generated files
...
- **新発見（先送りリスト外・grep掃引の成果）**: `docs.js` 生成物「注意点」節の **compat警告カウント `217ルール`/`217 rules` が陳腐化**（現行332）→ja+en修正。開始点ノート推奨の「生成物grep掃引から着手候補」が的中（既存の`332ルール`表記と整合、v9.40 Tier5と同型の鮮度バグ）
- **why_ja 350B超3件トリム（先送り「低優先」の消化）**: Netlify×Bun 457B→313B / Netlify×Express 387B→319B / Python logging(structlog) 530B→340B。意味保持（ランタイム非互換・ステートフル機能不可・OTel連携+PIIマスキング）で3件とも≤350Bに。build限界(6500KB)遵守。恒久ガードは`compat.test.js`側に既存（サイズ制約はビルド時実測）
- **CDN更新（v9.40 HOLD）— 2026-07再照会で HOLD 継続確定**: cdnjs は marked=16.3.0（v9.40から不変）/ mermaid=**11.12.0のまま**（11.13+のラベル互換修正・v11破壊的`mermaid.default.*`API未配信）。npm側mermaidは11.16だがcdnjs未達。marked 16は配信ありだがプレビュー専用の低価値＋v16でファイル配置変更(`marked.min.js`→`lib/marked.umd.js`)・グローバルAPI・SRI再計算のリスク。**外部条件（cdnjsのmermaid 11.13+配信）は未解消 → 据置が正**
- **GH Actions（先送り「v7余地」）— ノートの記述誤りを訂正 + 据置確定**: 2026-07照会で最新メジャーは checkout=**v6**・setup-node=**v6**（先送りノートの「v7」は誤り）。Node20は2026-06-02強制Node24化・09-16削除だが、**生成CIが既採用の`@v5`世代はNode24ベース＝非推奨対象外・破綻もEOLもなし**（v9.40がv5を"安全な現行世代"として選択したのは正しかった）。v6化は1メジャーの限界的鮮度×破壊的変更確認コスト（11ファイル×複数action）で低価値 → 据置。先送りノートの「v7」記述のみ「v6（現行最新・v5も安全）」に訂正

検証: npm run check（構文OK）+ node build.js（5958KB）+ 7525 tests 全合格。src変更は `docs.js`（カウント2箇所）+ `compat-rules.js`（why_ja 3件トリム）のみ。ルール総数332は不変（テスト追随不要）。教訓「先送りリストは着手前に外部条件を再照会して据置/実装を分離（CDN・GH Actionsは"最新化しない"が正だった）／grep掃引は新規バグを継続的に産む（217カウント）」
...
- **鮮度監査は外部照会(並列エージェント)+実データ突合が必須**: 知識カットオフ超の鮮度は記憶で断言せずWebSearch/context7で照会。**並列エージェントは各自が担当チャンクしか見えず「欠落」を誤報する**（v9.40: 偽gap14件=既存技術をgap誤報）→実データ全突合(*-all.json)を後処理必須化してから適用。改称も後継の既存チェック必須（既存なら改称=重複、削除が正しい帰結）
- **カウント/数値の鮮度は「実測で正値確定→置換」+ 同一数値の文脈分離**: 思い込みで置換しない（v9.40: 「134仕様書」はむしろ正・CLAUDE.mdの127が誤だった＝実生成カウントで判明）。同一数値でも文脈が別なら片方だけ置換（テンプレ数109 vs `docs/109`ファイル番号は隣接語で分離）
- **techdb等データ改称の下流波及チェック**: techdb選択→回答経路(`render.js _TECHDB_MAP`の8カテゴリ)のエントリ改称は、生成器の旧名分岐を孤立させる。改称後は`grep 旧名 src/generators`でロジック分岐(===/includes)を確認（v9.40: Remix→React Router v7でSSR判定漏れを修正）
