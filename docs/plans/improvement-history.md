...
**最終更新**: 2026-07-06 | **現在値**: v9.35 / 7493 tests / 5927KB (6500KB上限) / 116 launcher templates / 28 pillars / 227+ generated files
...
- **回答キー誤用の静的検査**（新・筆頭候補）: v9.35で6箇所見つかった `a.entities`/`a.features` 型の誤キー参照を、ジェネレータ全域で静的に禁止するテスト（`a\.(entities|features)\b` を build.test.js かユニットで拒否）。既知の正キー一覧（questions.jsのid集合）と突き合わせれば `a.deadline` 等の他キーも網羅可能。実装30分・確実な再発防止
...
- 完了済: ~~scope_out AI可視化~~(v9.30) | ~~scope_out why文検証~~(v9.29) | ~~doc番号レジストリ~~(v9.28) | ~~EN日本語根絶(かな)~~(v9.31) | ~~漢字のみEN掃引~~(v9.32) | ~~npm testフレーク解消~~(v9.33) | ~~全プリセット完全性掃引~~(v9.34) | ~~列挙生成検査の横展開(xref/Mermaid/フェンス)~~(v9.35)

**次回セッションの開始点**: この先送りリストの筆頭から再評価（着手前に価値を実測 — v9.26/v9.29の教訓）。回答キー静的検査は「questions.jsから正キー集合を抽出→ジェネレータの `a.xxx` 参照を全grep→差集合を確認」が最速の価値実測。
