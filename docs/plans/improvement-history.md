...
- **列挙生成検査の横展開**（筆頭候補）: v9.34方式（全プリセット×言語×スケール実生成→全数スキャン）を別性質へ。候補: (a) xref整合の全プリセット版（v9.25は代表シナリオのみ; postGenerationAudit C13相当を掃引側で全数実行） (b) Mermaid有効性の全プリセット版（C14相当; mermaid.test.jsは28柱×代表のみ） (c) 見出し構造/コードフェンス閉じ忘れ等のmd構文健全性。scripts/sweep-preset-integrity.js のCHECKS配列に検出器を足すだけで実装可能
- **launcher.js ja/en 圧縮**（393KB）: サイズ逼迫時に着手（現在573KB余裕）。v9.22で確立した共有ブロック方式の全面展開
- **7アンチパターン明示チェックリスト**: 参考資料の残り題材（既存設計と重複多く保留中）
- **why_ja 350B超過3件のトリム**: Netlify Functions×2 + Python logging（387〜530B）。機能影響なし・低優先
- 完了済: ~~scope_out AI可視化~~(v9.30) | ~~scope_out why文検証~~(v9.29) | ~~doc番号レジストリ~~(v9.28) | ~~EN日本語根絶(かな)~~(v9.31) | ~~漢字のみEN掃引~~(v9.32) | ~~npm testフレーク解消~~(v9.33) | ~~全プリセット完全性掃引~~(v9.34)

**次回セッションの開始点**: この先送りリストの筆頭から再評価（着手前に価値を実測 — v9.26/v9.29の教訓。列挙生成の横展開は「まず既存スクリプトに検出器1本足して掃引→ヒット数で価値判断」が最速）。
