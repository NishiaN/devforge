...
- **並列修正はソースファイル排他分担**: v9.31で6エージェント並列成功。インベントリ(JSON)先行生成→各自grep特定→「JA byte-identical維持+EN側のみ追加」の厳格ルール+担当外は報告のみ
...
- **npm test フレーク解消**（筆頭候補・小粒）: build.test.js の再ビルド（`execSync('node build.js')`）と security.test.js 等のバンドル読取が並行実行で競合し、タイミング次第で11件連鎖失敗する（v9.32で2回連続再現→再実行で解消・スタンドアロンは常時合格）。対策案: build.test.js を一時ファイルへのビルドに変える / バンドル読取系テストを遅延読み込みにする / テストファイル実行順の制御
...
- 完了済: ~~scope_out AI可視化~~(v9.30) | ~~scope_out why文検証~~(v9.29) | ~~doc番号レジストリ~~(v9.28) | ~~EN日本語根絶(かな)~~(v9.31) | ~~漢字のみEN掃引~~(v9.32)

**次回セッションの開始点**: この先送りリストの筆頭から再評価（着手前に価値を実測 — v9.26/v9.29の教訓）。
