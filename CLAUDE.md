...
**AI Development OS** — 62 JS modules in `src/` → single `devforge-v9.html` (~2908KB / 3000KB limit).
...
npm test                               # 1360 tests, all passing
...
| Gen quality | gen-quality (Suites 1-54, 522 tests) | ~522 |
| Preset matching | phase-n (N-1〜N-9 + G-1〜G-7, 68 tests) | ~68 |
| Other | i18n, state, techdb, utils, complexity, mermaid, help-hints | ~46 |

**Total: 1360 tests** | Test harness pattern: `eval(fs.readFileSync(...))` to load src files; global `S` mock at top.
