...
npm test                               # 2796 tests, all passing
...
| `data/` | 93 standard presets (`PR`/`_mp()`), 364 field presets (`PR_FIELD`/`_fpd()`), questions, techdb (358 entries), compat-rules (182 rules), gen-templates (bilingual GT dict), helpdata |
...
| Gen quality | gen-quality (Suites 1-151, ~1953 tests) | ~1953 |
| Preset matching | phase-n (N-1〜N-9 + G-1〜G-7, 68 tests) | ~68 |
| Other | i18n, state, techdb, utils, complexity, mermaid, help-hints | ~46 |

**Total: 2796 tests** | Test harness pattern: `eval(fs.readFileSync(...))` to load src files; global `S` mock at top.
