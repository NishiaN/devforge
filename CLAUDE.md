...
**AI Development OS** — 63 JS modules in `src/` → single `devforge-v9.html` (~3118KB / 5000KB limit).
...
npm test                               # 1546 tests, all passing
...
| `data/` | 63 standard presets (`PR`/`_mp()`), 266 field presets (`PR_FIELD`/`_fpd()`), questions, techdb (338 entries), compat-rules (142 rules), gen-templates (bilingual GT dict), helpdata |
...
File: `src/data/compat-rules.js` — currently 142 rules (24 error + 86 warn + 32 info). All rules have `why_ja`/`why_en`.
...
| Gen quality | gen-quality (Suites 1-69, 707 tests) | ~707 |
| Preset matching | phase-n (N-1〜N-9 + G-1〜G-7, 68 tests) | ~68 |
| Other | i18n, state, techdb, utils, complexity, mermaid, help-hints | ~46 |

**Total: 1546 tests** | Test harness pattern: `eval(fs.readFileSync(...))` to load src files; global `S` mock at top.
