...
npm test                               # 1601 tests, all passing
...
| `data/` | 63 standard presets (`PR`/`_mp()`), 266 field presets (`PR_FIELD`/`_fpd()`), questions, techdb (338 entries), compat-rules (152 rules), gen-templates (bilingual GT dict), helpdata |
...
File: `src/data/compat-rules.js` — currently 152 rules (26 error + 94 warn + 32 info). All rules have `why_ja`/`why_en`.
...
| Gen quality | gen-quality (Suites 1-75, 762 tests) | ~762 |
| Preset matching | phase-n (N-1〜N-9 + G-1〜G-7, 68 tests) | ~68 |
| Other | i18n, state, techdb, utils, complexity, mermaid, help-hints | ~46 |

**Total: 1601 tests** | Test harness pattern: `eval(fs.readFileSync(...))` to load src files; global `S` mock at top.
