...
describe('[SkillLevel] templateOrder count', () => {
  test('launcher templateOrder has 57 entries', () => {
    assert.ok(templateOrder !== null, 'templateOrder should be parseable from launcher.js');
    assert.strictEqual(templateOrder.length, 57, `templateOrder.length should be 57, got ${templateOrder ? templateOrder.length : 'null'}`);
  });
});
