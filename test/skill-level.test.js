...
describe('[SkillLevel] templateOrder count', () => {
  test('launcher templateOrder has 86 entries', () => {
    assert.ok(templateOrder !== null, 'templateOrder should be parseable from launcher.js');
    assert.strictEqual(templateOrder.length, 86, `templateOrder.length should be 86, got ${templateOrder ? templateOrder.length : 'null'}`);
  });
});
