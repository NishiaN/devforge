...
describe('[SkillLevel] templateOrder count', () => {
  test('launcher templateOrder has 115 entries', () => {
    assert.ok(templateOrder !== null, 'templateOrder should be parseable from launcher.js');
    assert.strictEqual(templateOrder.length, 115, `templateOrder.length should be 115, got ${templateOrder ? templateOrder.length : 'null'}`);
  });
});
