...
describe('[SkillLevel] templateOrder count', () => {
  test('launcher templateOrder has 103 entries', () => {
    assert.ok(templateOrder !== null, 'templateOrder should be parseable from launcher.js');
    assert.strictEqual(templateOrder.length, 103, `templateOrder.length should be 103, got ${templateOrder ? templateOrder.length : 'null'}`);
  });
});
