...
describe('[SkillLevel] templateOrder count', () => {
  test('launcher templateOrder has 54 entries', () => {
    assert.ok(templateOrder !== null, 'templateOrder should be parseable from launcher.js');
    assert.strictEqual(templateOrder.length, 54, `templateOrder.length should be 54, got ${templateOrder ? templateOrder.length : 'null'}`);
  });
});
