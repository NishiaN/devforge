...
describe('[SkillLevel] templateOrder count', () => {
  test('launcher templateOrder has 60 entries', () => {
    assert.ok(templateOrder !== null, 'templateOrder should be parseable from launcher.js');
    assert.strictEqual(templateOrder.length, 60, `templateOrder.length should be 60, got ${templateOrder ? templateOrder.length : 'null'}`);
  });
});
