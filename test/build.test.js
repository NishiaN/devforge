...
describe('Build System', () => {
  ...
  it('18 pillars consistency across all references', () => {
    const html = fs.readFileSync(OUTPUT, 'utf-8');
    // Check hero description has 18 pillars
    assert.ok(html.includes('18の柱') || html.includes('17の柱'), 'Should contain "18の柱" or "17の柱" in Japanese hero');
    assert.ok(html.includes('18 Pillars') || html.includes('18 pillars') || html.includes('17 Pillars') || html.includes('17 pillars'), 'Should contain "18 Pillars" or "17 Pillars" in English');
    // Check pillar arrays have 18 items
    const pillarJA = html.match(/pillar:\[([^\]]+)\]/);
    if (pillarJA) {
      const items = pillarJA[1].split(',').length;
      assert.ok(items === 18, `Pillar array should have 18 items, got ${items}`);
    }
