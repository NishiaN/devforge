...
**「質問に答えるだけで、AIが読める開発設計書を178+ファイル自動生成するツール」**
...
| 2 | MCP設定自動生成 | × | × | × | **○** |
| 3 | 32ドメインコンプライアンス | × | × | × | **○** |
| 4 | オフライン単一HTML | × | × | × | **○（3091KB）** |
| 5 | プリセット数 | ~5 | 0 | ~3 | **329種** |
| 6 | 技術互換性ルール（Why?付き） | 0 | 0 | 0 | **118ルール** |
| 7 | スキルレベルシステム | × | × | × | **7段階** |
| 8 | UI/生成言語独立バイリンガル | × | × | × | **○** |
| 9 | 設計書一括生成 | 0ファイル | 0ファイル | ~3ファイル | **178+ファイル** |
...
4. **オフライン単一HTML**: APIキー・サーバー・課金不要。エアギャップ環境でも動作。3091KBの単一ファイルがすべての機能を内包。

5. **329プリセット**: 63標準＋266フィールド、44カテゴリ×各2〜4種。ワンクリックで25問の回答が完了。

6. **118互換性ルール**: 全ルールに「▶ なぜ？」展開カード付き。ERRORは生成ブロック（ガードレール設計）、WARNは承知して続行可能。
...
9. **178+ファイル一括生成**: 25柱の設計書・設定・ドキュメントを10〜30秒で生成。bolt.newが「コードを書く」なら、DevForgeは「AIがコードを書くための設計図」を生成する。
...
### 2-4. プリセットの選択肢を把握する（標準63種 + 専門分野266種）
...
**Vol.3（44既存カテゴリ × 各2種 = 88プリセット）**

> v9.6.0 で追加。Vol.1・Vol.2 の44カテゴリそれぞれに「スケール別バリエーション」2種ずつを追加。
> 20（基本）＋14（Vol.1）＋10（Vol.2）= 44カテゴリ × 各2種 = 88プリセット
> Solo / Startup / Business / Enterprise の4スケールから最適な設定が自動適用される。
...
[生成言語選択: 日本語 or English]
    ↓
[🚀 生成ボタン] → 25ステップで178+ファイル生成（約10〜30秒）
    ↓
[ファイルプレビュー・確認]
    ↓
...
```
📦 ZIP ダウンロード
全178+ファイルを一括ダウンロード。プロジェクトフォルダにそのまま展開して使う。
```
...
node build.js
# → ✅ Built devforge-v9.html (3091KB, 63 modules)
...
**ビルドの制約**:
- ファイルサイズ上限: 3500KB（超えると`process.exit(1)`）
- 現在: ~3091KB（余裕: 約409KB）
...
**原因**: 相性の悪い技術の組み合わせを選択した（118のcompatルールが検出：15 ERROR + 73 WARN + 30 INFO）
...
新プリセット追加            → test/presets.test.js（件数 63+n）
...
4. 現状 ~3091KB / 3500KB 上限（余裕: ~409KB）

警告ライン:
~3300KB: 警告表示
~3500KB: process.exit(1)でビルド失敗
```
...
| **設計書不在**: コードだけあって設計がない | 178+ファイルの構造化ドキュメントを自動生成 |
...
| 柱（生成カテゴリ）数 | **25** 柱 |
| 標準プリセット数 | **63** 種類 |
| 専門分野プリセット数 | **266** 種類（44カテゴリ × 各2〜4種） |
| テーマオーバーレイ | **6** 種類（ドメイン横断合成） |
| 対応ドメイン数 | **32** ドメイン |
| AIプロンプトテンプレート | **40** テンプレート |
| 技術互換性チェックルール | **118** ルール（15E+73W+30I、全ルールに「▶ なぜ？」説明あり） |
| スキルレベル | **7** 段階 (Lv.0〜6) |
| テスト数 | **1,370** テスト（全パス） |
| ビルドサイズ | **~3091KB**（単一HTML / 上限3500KB） |
| 対応AIツール | Cursor, Claude Code, Windsurf, Cline, Copilot, Kiro, Codex, Gemini + AI Hooks, Skills |
...
*最終更新: 2026-02-26 | バージョン: v9.6.0 | テスト: 1,370 / Build: ~3091KB | プリセット: 標準63 + 専門266 + テーマ6 | テンプレート: 40*
